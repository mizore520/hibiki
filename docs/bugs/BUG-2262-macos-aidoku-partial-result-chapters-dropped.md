...
- **后记（2026-09-18）**：macOS 的 Aidoku 宿主（`main.rs` 子进程 CLI）已整条移除，「两个 runtime 对等」的不变式不复存在，`aidoku_partial_result_parity_guard_test.dart` 随之退役；`embedded.rs` 里的 partial 合并实现与 Rust 单测保留。
