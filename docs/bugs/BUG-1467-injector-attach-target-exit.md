## BUG-1467 · 附着模式游戏退出后 injector 不结束
...
- **[x] ② 已加自动化测试** — `native/galgame_hook/tests/hold_process_lifecycle_test.cpp` 覆盖“目标运行/目标退出/null 句柄”；`hibiki/test/tools/gal_injector_lifecycle_and_build_lock_test.dart` 固定 attach 调用必须传目标句柄且源码不得恢复无限循环。
