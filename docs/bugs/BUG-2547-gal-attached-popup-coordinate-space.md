...
- **备注**：新增 `gal_attached_popup_placement_test.dart` 走真实 controller 的 start/lookup/render 路径，mock 原生通道；覆盖主窗 DPR 1.75、呈现屏 DPR 1.5、负屏幕原点、根卡避让与后续普通查词不继承残留尺寸限制。本批独立审查未发现明确 P0–P2 回归，最终验证见 [当前交接](../personal/GAL_LOOKUP_HANDOFF.md)。完整应用构建、原游戏显示器上的避让/点击以及 Magpie 实机验收由用户集中执行；未以离线结果宣称上屏已验收。
