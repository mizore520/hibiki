...
- **[x] ① 已修复（待实机）** — 停止操作在异步边界核对 operation generation；重叠的来源清理共享同一 Future，新附着须等旧清理完成，旧停止不能在新会话开始后发布 idle。旧retry不得取消或重排新计时器；旧poll/重绑不能清掉新任务在途标记，复用同一个engine也须校验poll generation。晚到资源/PCM升格同样校验代次。源码随第十二轮候选提交，见 [当前交接](../personal/archive/GAL_LOOKUP_HANDOFF.md)。
