...
- **`patches`**：读取 [PATCHES.md](PATCHES.md) 里“补丁条目”那张表，用通配符路径统计每个条目覆盖了多少个人改动，并按目录列出未登记的改动。初版登记了 6 个条目，覆盖 710 个个人改动文件中的 201 个；其余的按“同步时逐步补登”处理。
- **`sync-upstream`**：先 fetch，然后在 `codex/sync-upstream-<作者提交>-<日期>` worktree 里合并作者的更新，不动 `custom`。同步报告写进交接单，内容包括：
  - 按类别列出的冲突文件，每类附处理原则；
  - 双方都改过、但自动合并成功的文件；
  - 作者这次改到了哪些个人补丁条目；
  - 数据库 `schemaVersion` 是否变化。

  同一时间只允许一个同步任务。
- **`pr-branch`**：从作者当前的 `upstream/develop` 建 `pr/<主题>` 分支和 `.worktrees/pr-<主题>`，按顺序 cherry-pick 指定提交（拒绝合并提交），遇到冲突就停下，并说明怎么继续。完成后用 `personal-paths.txt` 检查分支里有没有个人路径，最后给出推送和 `gh pr create` 的命令，但不自己执行。
- 个人专属路径的唯一来源仍是 `personal-paths.txt`（推送钩子用），`PATCHES.md` 只用于统计和提示，不重复维护一份。
- 自测放在 `tool/personal/tests/upstream.tests.ps1`（32 项）。
- **独立审查后的修正**：
  - 补丁条目只作附加提示，冲突文件仍按代码处理。原来整文件套用条目的策略，会把同一文件里其他个人改动（例如 Little Busters 语音适配）一起丢掉。
  - 放弃的同步或 PR 任务，可以在 `cleanup` 里归档 claim。worktree 已删除的旧同步不再挡住新同步，重开时自动换名。
  - “双方都改过”的文件完整写进交接单，按类别分组，代码类排在最前。
  - 其他：作者已有的改动 cherry-pick 时自动跳过；`-Commits` 顺序颠倒直接拒绝；不再用 `-x`，并列出提交说明供审阅；新增 `-Resume`；表格解析对列数不对的行报警、识别对齐分隔行；`**/` 可以匹配零层目录；规则文件、技术文档、生成文件（含 `pubspec.lock`）分别归类；作者改动数据库定义时也提示；只列出双方都改过的文件所涉及的补丁条目。
- **复核后补修**：
  - cherry-pick 失败时，只有确实是空提交（停在进行中，且暂存区和 HEAD 完全相同）才自动跳过；其他失败一律停下报错，不再把真实改动当成“作者已有”丢掉。
  - 正在解冲突或有未提交改动的任务，不再被列为可以“放弃并归档 claim”；示例命令也不选这一类。
  - 类型“规则”只对 `.md` 生效。
- **已知限制**：同一时间只允许一个同步任务，但这个检查不是原子操作，两个 agent 同时发起同步时可能各建一个。实际使用中同步由你手动触发，风险很低。
