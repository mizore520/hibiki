...
- 新增《ひこうき雲の向こう側》校准截图采集失败案例：提示“未能取得完整的游戏画面”，尚无样本，OCR 模型已就绪。归入 BUG-2543 的采集失败类别，未确认与既有案例同根因；当前只记录，详见样本记录末尾。
...
- 最新集中反馈：明确以用户新图中的浅蓝半透明单字格填充作为高亮视觉参考；超分开启时查词仍失败，用户称重新打开后恢复，具体重开对象及关闭超分后的状态待确认。只记录，详见样本记录末尾。
- 新增按游戏识别特殊标点并显示小框的需求：区分字形大小与实际占位，不能让缩框挤动后续排版；当前只记录，具体契约与可行性待统一调查，详见样本记录末尾。
...
- 本轮统一修复（`ff0ec5605a`、`54b7aac9c3`）：截图捕获在 WGC 创建窗口项失败时增加一次有限的窗口重建重试，并把“源窗口被 WGC 拒绝且没有可验证 Magpie 输出映射”单独记为 `wgc_item_no_verified_presentation`。没有放宽 Magpie 的 Src/Dest 裁剪契约，因此黑边、受保护窗口或没有可验证输出映射的游戏仍会明确失败，需后续按日志决定是否增加有证据的后端。
