...
- 用户已撤回同一游戏四句截图的排版反馈，认为属于不规则排版，明确不再处理这组适配。BUG-2542 对应小节仅保留历史，不列待办、不补采样、不为它修改算法；继续面向常见规律排版。其他通用对齐、超分和悬停显示反馈仍保留。
- 新增界面简化反馈：用户质疑多样本价值及旧控件是否冗余。代码核对确认自动对齐读取整份样本，校准样本拟合、验证样本只检查，并非仅处理选中图；后续需讲清用途、精简重复提示与数值控件，手动备用路径未经核对不直接删除。详见样本记录；本轮未改程序。
