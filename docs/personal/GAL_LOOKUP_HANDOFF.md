...
- **最多对话/旁白两个可选校准位置，任意一个即可使用**：界面保留“对话”和“旁白”各一个位置，不再提供任意增加其他校准样本的流程；两者不是必填。只校准其中任意一个时，该套参数（含特殊字符宽度）供两类正文共用，不能因另一项空缺阻止应用或查词；仅当两类排版不同，才分别校准并自动选择各自参数。两套均已校准时，特殊字符宽度按对话/旁白分别保存，不相互覆盖，切换排版时一起选择；各套排版内字符格高度仍统一，不按标点笔画缩高。此项是新需求记录，现有多样本入口及运行期契约尚未修改。
- **附着偶发无反应（用户报告此前没有，疑似回归）**：在anemoi中点击附着后有时没有反应，停止监听后重新附着有时恢复。截图显示目标窗口可列出，但工作台实时台词为0、等待选择线程、有1条尚无文本的线程，内查词状态为suspended。尚未确认失败在附着请求、注入、线程/正文到达或界面状态同步中的哪一步；不因等待线程提示就归咎于用户漏选，也不把重附着偶尔恢复当作修复。待本轮反馈结束统一调查。
