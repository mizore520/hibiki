...
- **步骤**：
  1. `flow sync-upstream -Agent "<代理名>"`。它会做这些事：
     ...
     - 把冲突文件按类别列出，每类附处理原则，同时把报告写进交接单：

       | 类别 | 处理原则 |
       |---|---|
       | 规则文件 | 保留个人版，只吸收作者新增的技术事实 |
       | 生成文件 | 重新生成，不手工解 |
       | i18n 源 | 按 i18n 纪律合并 |
       | bug 登记 | 用 `tool/bug.dart` 处理 |
       | [个人补丁清单](PATCHES.md) 里登记过的文件 | 按该条目的策略处理 |
       | 其他代码 | 拿不准就问用户 |
     - 列出“双方都改过、但自动合并成功”的文件（这些文件语义可能已经不对，要复核）；
     - 列出作者这次改到了哪些个人补丁条目；
     - 比较数据库 `schemaVersion` 有没有变化。
...
- **同一时间只允许一个同步任务**。前一个没收尾时，`sync-upstream` 会拒绝再开。

## S4 给作者提 PR

- **用户会说**：“把这个提给作者”。
- **步骤**：
  1. 找出要提交的个人提交，按从旧到新排好。只挑普通提交，不挑合并提交；混有个人内容的提交先拆开。
  2. `flow pr-branch <主题> -Commits <提交1>,<提交2> -Description "<说明>"`。它会从作者当前的 `upstream/develop` 建 `pr/<主题>` 分支和 worktree（`.worktrees/pr-<主题>`），逐个 cherry-pick，然后检查个人专属路径。遇到冲突时它会停下，并说明怎么继续。
  3. 在 PR worktree 里验证（按作者的要求：定向测试、全量 `flutter analyze`）。
  4. 用户同意推送后，照命令输出里的推送命令执行，带 `push` 标记。钩子会再检查一次：分支必须基于作者代码，且不含个人路径。
  5. 把 PR 的标题和描述给用户看，同意后再用输出里的 `gh pr create` 命令建 PR。
