...
- **步骤**：
  1. 运行 `flow adopt <分支>` 预览，把提交列表、改动文件、冲突和核对项**原样给用户看**。
  2. 有冲突时，先在任务 worktree 里 `git merge custom` 解冲突、提交，再重新预览。
  3. 用户明确同意后，运行 `flow adopt <分支> -Apply -Message "<合并说明>"`，带 `adopt` 标记。它会检查主 checkout 在 `custom` 且干净，然后合入；如果改了钩子，会自动重装；claim 和交接单会归档。任务还要继续做时，加 `-KeepClaim`。
...
- **步骤**：
  1. 运行 `flow cleanup` 列出清单。每项会标注“可清理”或“只报告”，以及风险：有没有本机证据、有没有未提交改动、有没有未进作者仓库的提交。
  2. 把清单**原样给用户看**，让用户按编号确认。
  3. 运行 `flow cleanup -Apply -Items <编号,...>`。删除 worktree、分支或目录要带 `cleanup` 标记；只归档 claim 不需要标记。
