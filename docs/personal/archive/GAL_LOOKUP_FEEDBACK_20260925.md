...
- **未推送（用户决定作者测试期间不推）**：PR 推送后 SonarCloud 质量门失败，唯一问题 cpp:S1048——`LogDroppedClick`（`29a2a595f6` 加进 `CancelPointerGesture`）经 `DestroySurfaceWindow` 可达析构函数，`std::ostringstream` 理论上可抛。已修为 `noexcept` + `catch (...)` 并加源码守卫：本分支 `426f1a2ecb`，PR 工作区本地 `0f74a46995`（在 `2049f5a922` 之上，未推送）。作者有反馈或需要改 PR 时一并推送。`author-allowlist` 红叉是等作者打 `merge-approved` 的门，与代码无关。
