...
2026-09-25 集成更新：优化分支 `codex/gal-lookup-optimize-20260925` 已与包含 Gal Opus 语音适配的 `custom` 本地整合。两边功能代码无同文件冲突；`docs/BUGS.md` 已用仓库工具重建。合并后完整 Flutter 静态分析和查词、校准、音频索引定向测试 257 项通过。BUG-2680 按用户此前决定暂缓；本次未推送，未对合并后的完整 Windows 应用做游戏实测。下文优化轮文字记录合入前的候选阶段。

更新：2026-09-24，作者源码已整合进原候选；用户报告修复后完整编译及测试未见问题，并授权合入、推送 `custom`。完整历史见 [样本记录](../LOOKUP_CALIBRATION_SAMPLES.md)。

## 2026-09-25 优化轮（进行中）

新候选在 `.worktrees/gal-lookup-optimize-20260925`，分支 `codex/gal-lookup-optimize-20260925`，基于 `custom@47396e2a2c`。修改了校准界面，去掉「只用「」内台词」开关，修复关闭查词窗口后点击迟钝（BUG-2674/2675）和 Hook 自带换行的长句错位（BUG-2676）。目标是更新到草稿 PR hajisensai/Fushi#1625。第三轮修复后等待用户编译复测（制卡租约问题 BUG-2680 按用户决定暂缓）；未合入、未推送。问题、提交、证据和复测清单见 [本轮清单](GAL_LOOKUP_FEEDBACK_20260925.md)。下面各节是 2026-09-24 已采用候选的记录。
