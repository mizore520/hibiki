...
| 10 | 用户要求：统一修改时通查查词与校准相关的界面文案，不合适的一起改 | 范围：校准对话框、台词工作台查词栏与模式菜单、游戏内三点校准提示、采集 / 自动对齐失败提示（`game_lookup_samples_*`、`game_lookup_attached_*` 等）。检查点：与当前单图流程和实际控件一致（不再提不存在的功能）；颜色 / 控件名与界面一致；失败提示说出真实原因和下一步；用词统一、句子简短、面向普通用户。只改文案值，不改 key；非中文语言中仍为英文占位的同步修正英文 | 待统一修改 |
