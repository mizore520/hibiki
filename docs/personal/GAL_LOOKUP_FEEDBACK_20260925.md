...
| 10 | 用户要求：统一修改时通查查词与校准相关的界面文案，不合适的一起改 | 范围：校准对话框、台词工作台查词栏与模式菜单、游戏内三点校准提示、采集 / 自动对齐失败提示（`game_lookup_samples_*`、`game_lookup_attached_*` 等）。检查点：与当前单图流程和实际控件一致（不再提不存在的功能）；颜色 / 控件名与界面一致；失败提示说出真实原因和下一步；用词统一、句子简短、面向普通用户。只改文案值，不改 key；非中文语言中仍为英文占位的同步修正英文。已知例子：空状态用的 `game_lookup_samples_hint`「先采集几句不同台词，再调整排版」与每类只用 1 张截图不符（截图 `7.png`），应改为提示选两行以上的完整台词 | 待统一修改 |
| 3 补 | 候选 `254c741d7b` 复测：用户猜测点不开是因为「后端还没查到词」 | 日志否定了这个猜测：成功的点击从收到查词请求到查完只要 8～50 ms，窗口在此后几十毫秒内渲染（如 12:32:51「草」）。关闭窗口后恢复现在是 0.05～0.12 s（修改前 0.3～2.9 s）。真正的失败例：12:26:57 关窗后，12:26:59 与 12:27:00 两次点击都让拦截进入握手等待（`gal-shield: … (no new probe)`，约 350 ms，说明点击已被查词层接住），但没有 `lookupText`；12:27:01 第三次才查到「ふか」。点击在 `EndPointerGesture`（`attached_text_surface_window.cpp`）被**静默丢弃**，可能的条件：按下与松开之间移动超过 `SM_CXDRAG/2`（约 2 px）被判为拖动；松开时不在按下的同一个字上；两者之间 `text_generation_` 或 epoch 变化；按下时 `BeginPointerGesture` 找不到字或接不上拦截事务。这几条都没有日志，违反 SOP「点击已消费但没有发布 lookup 必须有明确 reason」。修改方向：为每种丢弃写 reason 日志；点击判定改为按下、松开在同一个字上即算点击，不再用 2 px 阈值 | 已定位到丢弃位置，具体是哪一条待日志确认；待统一修改 |
| 11 | 制卡时第一次点「制卡」失败，第二次成功 | 日志 12:34:24.667：`窗口截图失败：GalHookCaptureSuppressionException: the attached glyph surface is no longer current`，点击后 5 ms 即被拒；12:34:27.955 第二次成功（先进入 `captureSuppressed`，再写入 note）。制卡截图要先隐藏查词层，`GalAttachedTextController.acquireMiningCaptureLease` 只接受 `activeAttached`，或查词窗口占用鼠标的那一种状态（`mouseHookBusy` + `singleton_owned_by_other_hwnd`）。点制卡按钮本身会让拦截短暂进入 `input_shield_rehandshake_pending` / `conflicting_transaction_pending`，制卡请求撞上这个过渡状态就返回 null，被报成「不是当前状态」。这是既有问题：同一报错在 9-19 出现 8 次、9-24 出现 4 次，都早于本轮修改。修改方向：制卡截图只要求当前台词和档案身份一致，查词窗口开着时的输入过渡状态不应否决截图；失败时给出真实原因 | 已定位，待统一修改 |
