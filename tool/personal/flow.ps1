...
.DESCRIPTION
  ...
  adopt <分支> -Apply         合入 custom（需 FUSHI_APPROVE=adopt；-Message 合并说明，-KeepClaim 不归档 claim）
  cleanup                     列出可收尾的 worktree / 分支 / claim / 空目录（只读）
  cleanup -Apply -Items W1,C2 按编号执行（删除类需 FUSHI_APPROVE=cleanup）
...
switch ($Command) {
    ...
    'adopt' {
        ...
        if ($Apply) {
            Invoke-FlowAdopt $context $Name $Message -KeepClaim:$KeepClaim
        }
        else {
