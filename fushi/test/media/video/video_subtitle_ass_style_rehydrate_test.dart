...
void main() {
  test(
    ...
    '(TODO-1246 respect-ass-style rehydration mechanism)',
    () async {
      ...
      expect(
        firstStyle.primaryColorArgb,
        0xFFFF00FF,
      ); // BGR &H00FF00FF -> magenta
      ...
      expect(
        fromDb.single.text,
        isNotEmpty,
        reason: 'text content survives DB',
      );
      expect(
        fromDb.single.markup,
        isNull,
        reason:
            'markup is transient and lost on DB round-trip — this is why '
      ...
      expect(
        rehydrated.single.markup?.cueStyle?.fontName,
        'FOT-Matisse ProN B',
        reason:
            're-parsing the external file recovers cueStyle so the '
  ...
  test('_loadSingle re-parses external text subtitle files to recover markup '
    ...
    final int helperAt = src.indexOf(
      'String? _rehydratableExternalSubtitlePath(',
    );
    expect(
      helperAt,
  ...
  test('SubtitleSource.isEmbeddedPersisted only matches embedded:<n> sentinels '
    ...
    expect(
      SubtitleSource.isEmbeddedPersisted(SubtitleSource.offSentinel),
      isFalse,
    );
    ...
    expect(
      SubtitleSource.isEmbeddedPersisted(embedded.toPersistedValue()),
      isTrue,
    );
  });

  test('_loadSingle re-parses embedded text tracks (cache hit, no ffmpeg) to '
    ...
    expect(
      body.contains('SubtitleSource.isEmbeddedPersisted(externalSub)'),
      isTrue,
      reason: 'embedded rehydration must gate on embedded:<n> persisted',
    );
    expect(
      body.contains('cues.isNotEmpty'),
      isTrue,
      reason:
