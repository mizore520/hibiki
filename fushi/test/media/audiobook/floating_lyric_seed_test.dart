...
void main() {
  ...
  void installPlatform() {
    const MethodChannel sessionCh = MethodChannel(
  ...
  test('starting the overlay pushes the current cue via updateText, after show '
      '(seeds the native prefs replay)', () async {
    ...
    expect(
      ...
      isNonNegative,
      reason:
          'an updateText carrying the current cue must follow show '
    ...
    expect(
      ...
      'cue 1000',
      reason:
          'the cue at positionMs=1000 is the current line seeded '
          'on the first frame',
    );
  });

  test(
    'updateText is not emitted when the floating overlay is disabled',
    () async {
      await startedSession(
        floatingOn: false,
        cues: <AudioCue>[cue(0)],
      ...
      expect(
        nativeCalls.any((MethodCall c) => c.method == 'updateText'),
        isFalse,
  ...
  test(
    'N=0 keeps single-line updateText with no-line-marker range (-1, 0)',
    () async {
      await startedSession(
        floatingOn: true,
        contextLines: 0,
  ...
  test(
    'N=1 pushes a 3-line context block with the real current-line range',
    () async {
      await startedSession(
        floatingOn: true,
        contextLines: 1,
