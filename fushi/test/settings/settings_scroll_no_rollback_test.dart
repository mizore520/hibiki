...
/// 守住 FushiFocusController 的 touch 门控根因（f6ef60d27：被动焦点修复 reveal 仅在
/// traditional 高亮模式生效，touch 下无光标 → 不 reveal）；本条用**真实
/// MaterialSettingsRenderer + 真实长 destination**，关闭「设置页专用」缺口 —— 证明真实
/// 设置列表的行（`_SettingsRowFocusTarget` 即 FushiFocusTarget）也吃这个门控：深滚后
...
void main() {
  ...
  Future<({ScrollController controller, Element listElement})> pumpSettings(
    ...
    addTearDown(() => ReaderFushiSource.readerSettings = prevReader);
