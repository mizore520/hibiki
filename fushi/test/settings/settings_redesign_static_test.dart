...
void main() {
  const Map<String, List<String>> requiredFiles = <String, List<String>>{
    ...
    'lib/src/settings/settings_section_container.dart': <String>[
      'class SettingsSectionContainer',
  ...
  test('settings shared actions use MD3 dialog chrome', () {
    final String actionsSource = readNormalizedSource(
  ...
  test('settings schema uses task-oriented destinations', () {
    final String destinationSource = readNormalizedSource(
  ...
  test('custom fonts are grouped with app appearance typography', () {
    ...
    final String readingSource = readNormalizedSource(
      'lib/src/settings/settings_schema_reading.dart',
    );
  ...
  test('reader quick settings project from schema reader placements', () {
    ...
    final String schemaSource = readNormalizedSource(
      'lib/src/settings/settings_schema.dart',
    );
    final String lookupSource = readNormalizedSource(
      'lib/src/settings/settings_schema_lookup.dart',
    );
    final String destinationSource = readNormalizedSource(
      'lib/src/settings/settings_destination.dart',
    ...
    for (final String itemId in <String>[
      ...
      expect(
        itemBody,
        contains('group: ReaderGroup.lookup'),
        reason: '$itemId 必须归查词分组',
      );
      expect(
        itemBody,
        isNot(contains('group: ReaderGroup.behavior')),
  ...
  test('reader quick settings reuse the shared theme selector', () {
    ...
    expect(
      containsIdentifierCall(sheetSource, 'buildReaderThemeChip'),
      isFalse,
    );
  });
  ...
  test('sync backup settings use standard schema rows for options', () {
    ...
    expect(
      enclosingCallOf(source, "id: 'sync.mode'").name,
      'SettingsCustomItem',
    );
    expect(
      enclosingCallOf(source, "id: 'sync.statistics'").name,
      'SettingsSwitchItem',
    );
    // 词典与本地音频源数据库不再是开关：一类一行的显式动作行
    // （`SettingsCustomItem` + `_AssetTransferMenuRow`），方向在行尾菜单里选。
    expect(
      enclosingCallOf(source, "id: 'sync.dictionary_transfer'").name,
      'SettingsCustomItem',
    );
    expect(
      enclosingCallOf(source, "id: 'sync.local_audio_transfer'").name,
      'SettingsCustomItem',
    );
  ...
  test('profile destination uses one picker row for the active profile', () {
    ...
    final String profilesSource = readNormalizedSource(
      'lib/src/settings/settings_schema_profiles.dart',
    );
    final String actionsSource = readNormalizedSource(
      'lib/src/settings/settings_actions.dart',
  ...
  test('profile switching waits for reader settings refresh', () {
    final String source = readNormalizedSource(
      'lib/src/profile/profile_view_model.dart',
    );
  ...
  test('settings lists and schema sections opt into contained surfaces', () {
    ...
    final String schema = readNormalizedSource(
    ...
    final String container = readNormalizedSource(
      'lib/src/settings/settings_section_container.dart',
    );
    expect(
      containsIdentifierCall(schema, 'SettingsSectionContainer'),
      isTrue,
      reason:
          'schema sections must delegate disclosure to the shared container',
    );
    expect(schema, contains('presentation: section.presentation'));
    expect(schema, contains('children: rows'));
    expect(
      containsIdentifierCall(container, 'AdaptiveSettingsSection'),
      isTrue,
    ...
    expect(
      containsIdentifierCall(material, 'AdaptiveSettingsSection'),
      isTrue,
  ...
  test(
    ...
    () {
      ...
      expect(
        shared,
        contains('color ?? tokens.surfaces.card'),
        reason:
      ...
      expect(
        shared,
        contains('tokens.spacing.rowHorizontal'),
  ...
  test('settings rows bound long text and inline controls for MD3 density', () {
    ...
    final String destination = readNormalizedSource(
      'lib/src/settings/settings_destination.dart',
    );
    ...
    expect(
      shared,
      contains('maxLines: subtitleMaxLines'),
      reason:
          'subtitle line count must come from the opt-in override, not a '
          'hard-coded clamp that truncates long help text on narrow screens',
    );
    expect(
      shared,
      isNot(contains('maxLines: kSettingsRowSubtitleMaxLines')),
    ...
    expect(
      destination,
      contains('this.controlBelow = true'),
      reason: 'schema segmented rows default to the readable below-label form',
    );
    ...
    for (final String banned in <String>[
      ...
      expect(
        containsIdentifierCall(shared, banned),
        isFalse,
        reason: 'settings_shared.dart must keep the shared MD3 row system',
      );
    }
    expect(
      containsIdentifierCall(shared, 'Card'),
      isFalse,
  ...
  test('unified settings detail shell is the single page chrome (TODO-317)', () {
    ...
    final String shell = readNormalizedSource(
      'lib/src/settings/settings_detail_page.dart',
    );
    ...
    for (final String path in <String>[
      ...
      expect(
        ...
        isTrue,
        reason: '$path must render through the unified settings detail shell',
      );
