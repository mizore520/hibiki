...
void main() {
  ...
  group('TODO-1069 / TODO-1070：总开关走语义意图入口（置位 + 拉/隐窗 + 写意图 pref）', () {
    test(
      ...
      () {
        ...
        final int fontIdx = schema.indexOf(
    ...
    test(
      ...
      () {
        ...
        final int idx = session.indexOf(
          'Future<void> _stopBackgroundSurfaces()',
        );
        expect(idx, greaterThanOrEqualTo(0));
  ...
  group('TODO-1072：悬浮字幕描述文案更明确', () {
    test('floating_lyric_hint 的 en/zh 值已更新为「悬浮当前播放字幕句」的明确表述', () {
      ...
      final String en = File('lib/i18n/strings_en.g.dart').readAsStringSync();
      final String zh = File(
        'lib/i18n/strings_zh_CN.g.dart',
      ).readAsStringSync();
      expect(en.contains(want['lib/i18n/strings.i18n.json']!), isTrue);
      expect(zh.contains(want['lib/i18n/strings_zh-CN.i18n.json']!), isTrue);
    });
