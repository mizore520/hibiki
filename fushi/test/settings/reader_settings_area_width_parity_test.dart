...
void main() {
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final double flushWidth = tester
          .getSize(find.byType(AdaptiveSettingsSection).first)
          .width;

      await tester.pumpWidget(variant(true));
      await tester.pumpAndSettle();
      final double insetWidth = tester
          .getSize(find.byType(AdaptiveSettingsSection).first)
          .width;
      ...
      expect(
        flushWidth,
        moreOrLessEquals(boxWidth, epsilon: 1.0),
  ...
  test(
    'renderer gates horizontal inset on insetHorizontally + sheet opts out',
    () {
      final String renderer = File(
        'lib/src/settings/material_settings_renderer.dart',
      ).readAsStringSync();
      expect(
        RegExp(
          r'insetHorizontally\s*\?\s*detailHorizontalInsets\(tokens\)\s*:\s*EdgeInsets\.zero',
        ).hasMatch(renderer),
        isTrue,
        reason:
            'material detail body horizontal inset must be gated on the flag',
      );
      ...
      expect(
        sheet,
        contains('insetHorizontally: false'),
        reason:
            'schema-projected sub-pages must let the renderer drop its inset',
      );
      expect(
        sheet,
        isNot(contains('detailHorizontalInsets')),
