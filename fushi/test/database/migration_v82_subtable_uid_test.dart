...
CREATE TABLE revealed_images (
  ...
  PRIMARY KEY (book_key, image_key)
          ...
          rawDb.execute(
            'INSERT INTO reader_positions (book_key, section_index, '
          ...
          rawDb.execute(
            'INSERT INTO bookmarks (book_key, section_index, '
          ...
          rawDb.execute(
            'INSERT INTO book_custom_css (book_key, relative_path, '
          ...
          rawDb.execute(
            'INSERT INTO revealed_images (book_key, image_key, '
  ...
  test('v82：epub 行换 uid、SRT 行照抄原键、css 孤儿清除、行数不丢、索引重建', () async {
    ...
    expect(db.schemaVersion, 102, reason: 'v82 = 四子表书键切 uid');
    ...
    expect(
      await count(db, 'book_custom_css'),
      1,
      reason: 'ghost-book 孤儿行被 INNER JOIN 清除',
    );
    final QueryRow css = await db
        .customSelect('SELECT * FROM book_custom_css')
        .getSingle();
    ...
    expect(
      await count(db, 'revealed_images'),
      2,
      reason: 'revealed_images 行数不丢',
    );
    final QueryRow revUidCount = await db
        .customSelect(
          'SELECT COUNT(*) AS c FROM revealed_images '
          "WHERE book_uid = 'uid-a-stable'",
        )
        .getSingle();
...
CREATE TABLE reader_positions (
  ...
  updated_at INTEGER NOT NULL
          ...
          rawDb.execute(
            'INSERT INTO epub_books (book_key, title) '
            "VALUES ('book-old', '旧书')",
          );
          rawDb.execute(
            'INSERT INTO reader_positions (book_key, section_index, '
