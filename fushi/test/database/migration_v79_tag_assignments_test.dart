...
void main() {
  Future<FushiDatabase> openV78Db({bool legacyVideoUidColumn = false}) async {
    final String videoUidColumn = legacyVideoUidColumn
        ? 'video_book_uid'
        : 'book_uid';
...
''');
  ...
  test('fresh 库由 onCreate 直接建出 tag_assignments，五 kind API 全通', () async {
    ...
    expect(
      (await db.getAllTagAssignments()).every(
        (TagAssignmentRow r) => r.addedAt > 0,
      ),
      isTrue,
  ...
  test('review5-2 回归：FK ON 下悬空 tag_id 不炸升级——过滤丢弃而非 abort '
