...
void main() {
  ...
  group('schema v66 migration', () {
    test('fresh DB (v66) has collection_relations table and '
      ...
      final List<QueryRow> columns = await db
          .customSelect('PRAGMA table_info(video_scrape_meta)')
          .get();
      expect(
    ...
    test('real v64->v66 creates collection_relations, adds episode_number, '
      ...
      // 新表真的可用（不只是建了个壳）。
      await db.replaceCollectionRelations(1, <CollectionRelationsCompanion>[
        relation(1),
      ]);
  ...
  group('CRUD', () {
    test('replaceCollectionRelations swaps whole edge set and orders by '
        'sortIndex', () async {
      final FushiDatabase db = await openDb();
      ...
      final List<CollectionRelationRow> first = await db.getCollectionRelations(
        id,
      );
      expect(
        first.map((CollectionRelationRow r) => r.title).toList(),
        <String>['A', 'B'],
      ...
      final List<CollectionRelationRow> second = await db
          .getCollectionRelations(id);
      expect(second, hasLength(1));
      expect(second.single.title, 'C');
      expect(second.single.relationType, 'sequel');
    });

    test('bindCollectionRelationTarget upgrades scrape-only edge to local '
      ...
      final CollectionRelationRow edge = (await db.getCollectionRelations(
        id,
      )).single;
      ...
      expect(
        (await db.getCollectionRelations(id)).single.targetCollectionId,
        target,
      );
    ...
    test('collectionIdsByScrapeSubject finds collections scraped from the '
        'same subject', () async {
      final FushiDatabase db = await openDb();
      ...
      expect(
        await db.collectionIdsByScrapeSubject('tmdb', '900'),
        isEmpty,
        reason: '不同源不算同条目',
      );
    ...
    test('unique key (collectionId, source, subjectId) rejects duplicate '
        'edges', () async {
      final FushiDatabase db = await openDb();
      final int id = await db.createMediaCollection('本篇');
      await db.replaceCollectionRelations(id, <CollectionRelationsCompanion>[
  ...
  group('foreign keys', () {
    ...
    test('deleting the bound target collection nulls targetCollectionId '
