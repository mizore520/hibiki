...
void main() {
  test(
    ...
    () async {
      ...
      // `replay_trace` 的候选排序键是 (优先级, 时间差)，所以 fixture 必须把
      // loopback 放得比 PCM **更靠近**台词：优先级一旦被抹平（重构成同权、或把
      // loopback 提到与 pcm 同级），赢的就变成 loopback，`galhook.py replay`
      // 立刻对不上 expected 而 exit 2。
      // 这条 fixture 原本是反过来摆的（pcm@9015 比 loopback@9019 更近），于是
      // 「PCM 赢」是离得近赢的——抹平优先级照样绿，等于没测。
      final int textAt = eventById('unity-mono-line')['timestamp_ms'] as int;
      final int pcmGap =
          (eventById('unity-mono-source-pcm')['timestamp_ms'] as int) - textAt;
      final int loopbackGap =
          (eventById('unity-mono-loopback')['timestamp_ms'] as int) - textAt;
      expect(
        loopbackGap.abs(),
        lessThan(pcmGap.abs()),
