...
void main() {
  test('popup.js 渲染收藏按钮并接 favoriteEntry / favoriteCheck 桥', () {
    ...
    expect(
      js,
      contains("className: 'inline-action-button favorite-button'"),
      reason:
          '必须创建 favorite-button 元素（TODO-1325 #4 挂 inline-action-button 图标基类）',
    );
    expect(
      js,
      contains("'favoriteEntry'"),
      reason: '点击应调 favoriteEntry 桥写入 FavoriteWords',
    );
    expect(
      js,
      contains('scheduleEntryStateCheck('),
      reason: '初始态查询应离开同步首屏渲染路径',
    );
    expect(
      js,
      contains("'favoriteCheck', { expression, reading }"),
      reason: '词条接近可见时仍应查 favoriteCheck 设收藏图标',
    );
    expect(
      js,
      contains('buttonsContainer.appendChild(createFavoriteButton('),
      reason: '收藏按钮要挂进词条头',
    );
  });
  ...
  test('popup.css 保留 .favorite-button 样式', () {
    ...
    expect(
      css,
      contains('.favorite-button.favorited'),
  ...
  test('mixin 默认书籍来源、提供收藏 handler 并把成功制卡计入统计', () {
    ...
    expect(
      containsIdentifierCall(src, 'recordMined'),
      isTrue,
      reason: '制卡成功应计入统计',
    );
    ...
    expect(
      containsCodeLine(src, 'onFavoriteEntry: onFavoriteEntry'),
      isTrue,
  ...
  test('BaseSourcePage（阅读器/有声书弹窗宿主）也接 favorite handler 并接进 layer', () {
    ...
    expect(
      src,
      contains('String get dictionarySourceType => kStatSourceBook'),
      reason: '阅读器/有声书默认归书籍统计',
    );
    expect(
      src,
      contains('Future<bool> onFavoriteFromPopup('),
      reason: '基类必须提供收藏切换 handler',
    );
    expect(
      src,
      contains('Future<bool> onFavoriteCheckFromPopup('),
      reason: '基类必须提供收藏状态查询 handler',
    );
    expect(
      src,
      contains('addFavoriteWord('),
      reason: '收藏必须真写穿 FavoriteWords 表，而非 UI 假动作',
    );
    expect(
      src,
      contains('onFavoriteEntry: onFavoriteFromPopup'),
      reason: '_buildPopupLayer 必须把收藏写入 handler 接进 layer',
    );
    expect(
      src,
      contains('onFavoriteCheck: onFavoriteCheckFromPopup'),
  ...
  test('视频页把来源覆写为 video（收藏/制卡落视频统计）', () {
    ...
    expect(
      src,
      contains('String get dictionarySourceType => kStatSourceVideo'),
    );
  });
}
