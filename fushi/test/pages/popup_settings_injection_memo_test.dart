...
void main() {
  ...
  group('memo hit', () {
    ...
    test('per-options slots do not thrash each other', () {
      ...
      final PopupStaticSettingsJs global1 = build(
        appModel,
      ...
      final PopupStaticSettingsJs global2 = build(
        appModel,
        options: const PopupSettingsOptions(globalLookup: true),
      );
      expect(
        identical(inApp1, inApp2),
        isTrue,
    ...
    test('整帧 buildPopupSettingsJs：词条负载夹在静态 head/tail 之间且随输入变化', () {
      ...
      const PopupSettingsOptions options = PopupSettingsOptions(
        globalLookup: true,
      );
      ...
      expect(
        tailMarker,
        greaterThan(entriesMarker),
        reason:
            '词典样式/自定义 CSS 的 tail 在词条负载之后（popup.js '
            '依此顺序着色）',
      );
      expect(
        composed.substring(entriesMarker, tailMarker),
        contains('語'),
    ...
    testWidgets(
      'global host cold payload carries static; hot payload is dynamic-only',
                ...
                if (!built) {
                  ...
                  final List<GlobalLookupFramePayload> payloads =
                      <GlobalLookupFramePayload>[
                        GlobalLookupFramePayload(
                          frame: const GlobalLookupFrame(
                            id: kGlobalLookupRootFrameId,
                            query: '語',
                            parentIndex: -1,
                            resultCount: 1,
                          ),
  ...
  group('memo invalidation', () {
    ...
    test('改主题：brightness change rebuilds the theme vars', () {
      final MemoAppModel appModel = MemoAppModel();
      final PopupStaticSettingsJs light = build(
        appModel,
        theme: ThemeData(brightness: Brightness.light),
      );
      final PopupStaticSettingsJs dark = build(
        appModel,
    ...
    // 就只有它会红。
    final List<
      ({String name, void Function(MemoAppModel model) mutate, String marker})
    >
    scalarCases =
        <
          ({
    ...
    for (final ({
      ...
      test('memo 键字段失效：${c.name}', () {
        ...
        final PopupStaticSettingsJs before = build(appModel);
        expect(
          identical(build(appModel), before),
          isTrue,
          reason: '前置：未改任何输入时 memo 必须命中（否则下面的失效断言假绿）',
        );
        expect(
          before.combined,
          isNot(contains(c.marker)),
        ...
        expect(
          ...
          isNot(before.revision),
          reason:
              '${c.name} 是 memo 键的一部分，变了必须重建 + 换 revision；'
              '漏比这个字段 = 用户改了设置弹窗不生效',
        );
    ...
    test('memo 键字段失效：wheelBindingsJson（词条滚轮绑定）', () {
      ...
      expect(
        ...
        isNot(before.revision),
        reason:
            '改键后弹窗必须收到新绑定——漏比 wheelBindingsJson '
            '会让「改了快捷键不生效」',
      );
    ...
    test('memo 键字段失效：popupKeyBindings（app 外制卡/发音键盘绑定）', () {
      final MemoAppModel appModel = MemoAppModel();
      appModel.shortcutRegistry.loadDefaults(TargetPlatform.windows);
      final PopupStaticSettingsJs before = build(
        appModel,
        options: const PopupSettingsOptions(globalLookup: true),
      );
      expect(
      ...
      final PopupStaticSettingsJs after = build(
        appModel,
        options: const PopupSettingsOptions(globalLookup: true),
      );
      expect(after.revision, isNot(before.revision));
      expect(after.head, contains('"key":"m"'));
      expect(after.head, contains('"shift"'));
    });
    ...
    test('memo 键字段失效：lookupAudioVolume（查词发音音量）', () async {
      ...
      expect(
        ...
        isNot(before.revision),
        reason:
            '音量是 memo 键的一部分，改了必须重建（否则弹窗里的发音'
            '一直用旧音量）',
      );
  ...
  group('stylesJson identity 前提（memo 正确性最脆的一环）', () {
    test('dictionaryStyles / dictionaryStylesJson 未变时返回同一实例', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'dictionaryStyles 不得每次返回防御性副本——memo 靠 map 身份判'
    ...
    test('_stylesCache 只允许整体重赋值，禁止就地 mutate', () {
      ...
      expect(
        mutation.hasMatch(src),
        isFalse,
        reason:
            '就地修改不换 map 身份 → dictionaryStylesJson() 不重编码 → '
      ...
      final String rebuild = src.substring(
        rebuildAt,
        src.indexOf('\n  }', rebuildAt),
      );
      expect(
        RegExp(r'_stylesCache\s*=\s*\{').allMatches(rebuild).length,
        2,
    ...
    test('注入侧的 identical 判据与编码缓存键没被改成内容比较', () {
      ...
      expect(
        injection,
        contains('identical(cached.stylesJson, stylesJson)'),
        reason:
            '词典样式 JSON 可能是 MB 级，memo 命中判据必须是身份比较；'
            '改成 == 内容比较就把本次性能修复抵消了',
      );
      ...
      final int jsonAt = webview.indexOf(
        'static String dictionaryStylesJson()',
      );
      expect(jsonAt, greaterThanOrEqualTo(0));
      final String body = webview.substring(
        jsonAt,
        webview.indexOf('\n  }', jsonAt),
      );
      expect(
        body,
        contains('identical(styles, _cachedStylesRef)'),
        reason:
            'JSON 编码缓存的失效键就是 styles map 的身份（与 '
  ...
  group('换字体：dictionary font JS memo（(name,path,mtime,size) 指纹键）', () {
    ...
    setUp(() async {
      ...
      final Directory fontsDir = await Directory(
        '${tempDir.path}/custom_fonts',
      ).create();
    ...
    test('unchanged font state returns the identical JS instance', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            '同 (name,path,mtime,size) 必须命中 memo 返回同一实例——'
    ...
    test('overwriting the font file (mtime bump) rebuilds font JS and the '
      ...
      await fontFile.setLastModified(
      ...
      expect(
        staticAfter.revision,
        isNot(staticBefore.revision),
