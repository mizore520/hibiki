...
void main() {
  ...
  test('play 钉几何中心：三区 Stack（左时间 / 右尾部 / Center 居中 seek 簇）', () {
    ...
    expect(
      helper.contains('Center(child: transport)'),
      isTrue,
      reason: 'seek 传输簇应绝对居中（play 钉几何中心）',
    );
    // 左区时间 / 右区尾部按钮各自绝对对齐，不靠 Spacer 均分挤偏 play。
    expect(
      helper.contains('alignment: Alignment.centerLeft'),
      isTrue,
      reason: '时间指示器应左对齐绝对定位',
    );
    expect(
      helper.contains('alignment: Alignment.centerRight'),
      isTrue,
      reason: '尾部按钮应右对齐绝对定位',
    );
    // 旧的 Spacer 平铺布局已从共享 helper 移除（不再用 Spacer 定位 seek 簇）。
    expect(
      helper.contains('Spacer()'),
      isFalse,
  ...
  test('seek 按钮带可见标注（±10s），不只有 tooltip', () {
    ...
    expect(
      helper.contains('VideoControlSlot.bottomCenter'),
      isTrue,
      reason: '居中 transport 应从 bottomCenter slot 渲染',
    );
    expect(
      slotButtonBuilder.contains('label: t.video_bottom_seek_back_label'),
      isTrue,
      reason: '−10s 按钮应带可见标注',
    );
    expect(
    ...
    expect(
      src.contains('Widget _seekLabelButton('),
      isTrue,
      reason: '应有带可见标注的 seek 按钮构造器',
    );
    // i18n label key 已存在于生成文件。
    final String gen = File('lib/i18n/strings_en.g.dart').readAsStringSync();
