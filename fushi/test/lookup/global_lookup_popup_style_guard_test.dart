...
void main() {
  ...
  group('scope marker', () {
    test('the shared builder injects the global-lookup class (gated)', () {
      ...
      expect(
        render.contains('PopupSettingsOptions(globalLookup: true)'),
        isTrue,
        reason: 'the app-outside frame must request the global-lookup options',
      );
    });

    test(
      'in-app dictionary_popup_webview NEVER adds the global-lookup class',
      () {
        ...
        expect(
          ...
          isFalse,
          reason:
              'the in-app popup must NOT tag itself global-lookup — its card '
  ...
  group('popup.css scoping (source)', () {
    ...
    test('card chrome is scoped to html.global-lookup body, not bare body', () {
      expect(
        css.contains('html.global-lookup body {'),
        isTrue,
      ...
      expect(
        bareBody.contains('border-radius'),
        isFalse,
    ...
    test(
      ...
      () {
        ...
        expect(
          gridRule.contains('display: grid'),
          isTrue,
          reason: 'the tested --dict-columns grid must remain for in-app',
        );
        expect(
          gridRule.contains('align-items: start'),
          isTrue,
          reason:
              'align-items:start 让所有面（含 app 外）取自然高度、免 grid stretch，'
        ...
        expect(
          css.contains(
            'html.global-lookup .glossary-section > .category-body {',
          ),
          isFalse,
          reason:
              'global-lookup 不得再用独立 flex-wrap 覆盖 .category-body — '
        ...
        expect(
          ...
          isFalse,
          reason:
              'global-lookup 的 glossary-group flex 覆盖（flex:1 1 auto; '
  ...
  group('P3c F1 — icon glyph fix (route A: monochrome symbol font)', () {
    ...
    test('global-lookup iframe forces the monochrome Segoe UI Symbol font', () {
      ...
      // colour emoji.
      expect(
        inject.contains('"Segoe UI Symbol"'),
        isTrue,
    ...
    test('the emoji font is DROPPED (no colour-emoji rendering of ♪/✕)', () {
      ...
      expect(
        inject.contains('Segoe UI Emoji'),
        isFalse,
    ...
    test(
      'the icon font is applied to the audio button + collapse arrow glyph',
      () {
        // .audio-button = ♪ ; .glossary-group>summary::before = ▶/▼ content glyph.
        expect(inject.contains('.audio-button'), isTrue);
        expect(
          inject.contains('.glossary-group>summary::before'),
          isTrue,
          reason:
              'the arrow glyph lives in the ::before content, so the font '
              'override must target the pseudo-element',
        );
      },
    );

    test('the icon override is gated behind options.globalLookup (in-app never)', () {
      ...
      // globalLookup:true.
      expect(
        inject.contains('options.globalLookup ? _globalLookupIconFontJs'),
        isTrue,
      ...
      final String inApp = read(
        'lib/src/pages/implementations/dictionary_popup_webview.dart',
      );
      expect(
        inApp.contains('Segoe UI Symbol'),
        isFalse,
        reason: 'in-app popup must not carry the global-lookup icon override',
      );
    });
  });
  ...
  group('P3c F2 — outer shell chrome (.global-lookup-frame-shell)', () {
    ...
    test('shell base carries radius and no box-shadow', () {
      ...
      expect(
        host.contains('border-radius:10px'),
        isTrue,
    ...
    test('shell receives the single computed border at runtime', () {
      ...
      expect(
        host.contains('border:1px solid rgba(120,120,128,0.36)'),
        isFalse,
        reason: 'host must not duplicate the theme border literal',
      );
      expect(
        host.contains('border-color:rgba(255,255,255,0.34)'),
        isFalse,
      ...
      expect(
        host.contains("'background:transparent!important;'"),
        isTrue,
        reason: 'once transferred, body must not paint a duplicate fill',
      );
      expect(
        host.contains("'border-color:transparent!important;}'"),
        isTrue,
        reason: 'body keeps border layout but must not paint a second ring',
      );
      expect(
        host.contains('.global-lookup-frame-shell::after{'),
        isTrue,
        reason: 'the one visible border belongs to a non-layout shell layer',
      );
      expect(
        host.contains('var(--global-lookup-shell-border-width,0px)'),
        isTrue,
      );
      expect(
        host.contains("shell.style.border = '0';"),
        isTrue,
    ...
    test('popup.css remains the theme source for the transferred border', () {
      ...
      expect(
        bodyRule.contains('border: 1px solid #D1D1D6'),
        isTrue,
    ...
    test('shell stamps data-theme from the render payload (no themed shadow)', () {
      ...
      expect(
        host.contains('.global-lookup-frame-shell[data-theme="dark"]{'),
        isFalse,
        reason:
            'the dark box-shadow shell variant is gone (no shell-level '
            'themed rule left)',
      );
      expect(
        host.contains('descriptor && descriptor.theme'),
        isTrue,
        reason: 'host.js must still stamp data-theme from the descriptor',
      );
      final String render = read('lib/src/lookup/global_lookup_render.dart');
      expect(
        render.contains("map['theme'] ="),
        isTrue,
    ...
    test('the shell chrome stays global-lookup scoped (host.js only)', () {
      ...
      expect(
        host.contains('window.top !== window.self'),
        isTrue,
  ...
  group('native rounded window (C++)', () {
    test('global_lookup_window rounds the opaque window via SetWindowRgn', () {
      ...
      expect(
        cpp.contains('CreateRoundRectRgn'),
        isTrue,
        reason:
            'opaque non-layered window needs a region for real round '
  ...
  group('TODO-895 — settings injection is a single source of truth', () {
    ...
    test('both call sites go through the shared settings builders', () {
      ...
      // output). Both hot paths use its shared static half
      // (buildPopupStaticSettingsJs) + per-lookup entries half
      // (buildPopupEntriesJs) to dedup the persistent WebView payload — the
      // SAME shared builders buildPopupSettingsJs composes, so
      // single-source-of-truth still holds.
      expect(
        inject.contains('String buildPopupSettingsJs('),
        isTrue,
      ...
      expect(
        inject.contains('String buildPopupEntriesJs('),
        isTrue,
        reason: 'the shared per-lookup entries builder must exist',
      );
      expect(
        render.contains('buildPopupStaticSettingsJs('),
        isTrue,
        reason: 'the app-outside frame must call the shared static builder',
      );
      expect(
        render.contains('buildPopupEntriesJs('),
        isTrue,
        reason: 'the app-outside frame must call the shared entries builder',
      );
      expect(
        inApp.contains('buildPopupStaticSettingsJs('),
        isTrue,
        reason: 'the in-app popup must call the shared static-settings builder',
      );
      expect(
        inApp.contains('buildPopupEntriesJs('),
        isTrue,
    ...
    test('D1 — the dictionary font is injected by the shared builder', () {
      ...
      expect(
        inject.contains("id = 'fushi-dict-font'"),
        isTrue,
        reason: 'the shared builder must inject the dictionary font style',
      );
      expect(
        inject.contains('DictionaryFontCss.build('),
        isTrue,
      ...
      expect(
        render.contains('fushi-dict-font'),
        isFalse,
    ...
    test('D2 — autoExpandRows is in the shared body', () {
      ...
      expect(
        inject.contains('window.autoExpandRows ='),
        isTrue,
      ...
      // be duplicated there).
      expect(
        render.contains('window.autoExpandRows ='),
        isFalse,
    ...
    test('D3 — content zoom uses the clamped/NaN-guarded helper only', () {
      ...
      expect(
        inject.contains('DictionaryPopupWebViewState.popupContentZoom('),
        isTrue,
        reason: 'the shared body must use the clamped zoom helper',
      );
      expect(
        render.contains('appUiScale * ('),
        isFalse,
        reason: 'app-outside must not compute zoom inline (drift source)',
      );
      expect(
        inject.contains('appModel.appUiScale * ('),
        isFalse,
  ...
  group('TODO-893 v2 — three reopened symptoms (source guards)', () {
    test(
      'symptom 1 — controller dispatches BOTH onLinkClick and textSelected',
      () {
        ...
        expect(
          controller.contains("handler == 'textSelected'"),
          isTrue,
          reason: 'controller must handle the textSelected (body tap) trigger',
        );
        expect(
          controller.contains("handler == 'onLinkClick'"),
          isTrue,
          reason: 'onLinkClick (headword/kanji) must still dispatch',
        );
        expect(
          controller.contains('_dispatchNestedLookup('),
          isTrue,
    ...
    test('symptom 2 — global lookup keeps a transparent canvas background', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'html.global-lookup needs a real but fully transparent layer so '
      ...
      expect(
        host.contains(
          "'background', FRAME_CANVAS_GUARD_BACKGROUND, "
          "'important'",
        ),
        isTrue,
      );
    ...
    test(
      'symptom 3 — controller feeds selectionScreenOffset to the builder',
      () {
    ...
    test(
      'symptom 3 — render builder shifts anchor in then shifts result out',
      () {
        ...
        final String render = read('lib/src/lookup/global_lookup_render.dart');
        expect(
          render.contains('Offset selectionScreenOffset'),
          isTrue,
          reason: 'builder must accept the offset param',
        );
        expect(
          render.contains('anchorRect.shift(selectionScreenOffset)'),
          isTrue,
          reason: 'anchor lifted into work-area-absolute domain before layout',
        );
        expect(
          render.contains('r.left - selectionScreenOffset.dx'),
          isTrue,
          reason: 'result shifted back to window-local (X)',
        );
        expect(
          render.contains('r.top - selectionScreenOffset.dy'),
          isTrue,
  ...
  group('BUG-859 — overlay cascade is ALWAYS horizontal (supersedes TODO-938)', () {
    ...
    test('controller does not read writingMode for the cascade', () {
      ...
      expect(
        ...
        isFalse,
        reason:
            'the writingMode→cascade wiring (BUG-453) must stay removed — '
      ...
      expect(
        controller.contains('isVertical:'),
        isFalse,
    ...
    test('render anchors nested cards with isVertical: false', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'computeFrameRect must be fed the horizontal cascade '
            '(anchors are words inside a horizontal popup card)',
      );
      expect(
        render.contains('isVerticalFromWritingMode'),
        isFalse,
        reason: 'the writingMode helper must stay deleted (dead wiring)',
      );
      expect(
        render.contains('this.isVertical'),
        isFalse,
    ...
    test('controller converts the work-area domain with the MONITOR dpr', () {
      ...
      expect(
        flat.contains('shown.workWidth / workDpr'),
        isTrue,
        reason: 'workWidth must be divided by the monitor dpr',
      );
      expect(
        flat.contains('shown.cursorWorkX / workDpr'),
        isTrue,
    ...
    test('native showAt reports the anchor monitor dpr', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'showAt must report the cursor monitor dpr for the CSS px '
            'conversion (BUG-859 mixed-scale multi-monitor)',
      );
      expect(
        cpp.contains('"monitorDpr"'),
        isTrue,
    ...
    test(
      'native RevealStack folds the work-area clamp into the layer shift',
      () {
        ...
        expect(
          flat.contains('bbox_left + clamp_dx_css'),
          isTrue,
          reason: 'the X clamp delta must be folded into commitLayerShift',
        );
        expect(
          flat.contains('bbox_top + clamp_dy_css'),
          isTrue,
  ...
  group('TODO-1067 — desktop overlay nested popup close wiring (source)', () {
    ...
    test('子4/TODO-1231 — __hasChildPopup rides its own channel (BUG-434 kept)', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'a frame has a child iff it is not the last in the stack, '
      ...
      expect(
        host.contains('function applyHasChildPopup('),
        isTrue,
        reason: 'the host applies __hasChildPopup on its own cheap channel',
      );
      final int applyStart = host.indexOf('function applyHasChildPopup(');
      final int applyEnd = host.indexOf(
        '\n  function clearFrameSelection',
        applyStart,
      );
      expect(applyEnd, greaterThan(applyStart));
      final String applyBody = host.substring(applyStart, applyEnd);
      expect(
        applyBody.contains('win.__hasChildPopup = desired;'),
        isTrue,
        reason:
            'the same-origin host must assign the boolean directly in the '
            'frame realm without paying another eval round-trip',
      );
      expect(
        applyBody.contains('.eval('),
        isFalse,
        reason:
            'nested open/close must not reintroduce an iframe eval for this '
            'single boolean',
      );
    });
    ...
    test('子2 — buildFrameSettingsJs injects the shared swipe-close JS', () {
      ...
      expect(
        render.contains(r'$kPopupTopPullReleaseJs'),
        isTrue,
      ...
      expect(
        ...
        isTrue,
        reason:
            'swipe close stays preference-gated (close-X is the primary '
    ...
    test('子1 — host.js draws a per-shell close-X posting dismissPopupAt[index]', () {
      expect(
        host.contains('function createCloseButton('),
        isTrue,
      ...
      expect(
        ...
        isTrue,
        reason: 'the X dismisses this layer by its stack index, not the root',
      );
      expect(
        host.contains(
        ...
        isTrue,
        reason:
            'a reused physical shell must close its current monotonic frame id',
      );
      expect(host.contains('var index = layerIndexOf(liveFrameId);'), isTrue);
      // The close-X CSS is present in the injected gate/shell stylesheet.
      expect(
        host.contains('.global-lookup-close{'),
        isTrue,
    ...
    test(
      '子5 — host DEFERS card clicks to popup.js; only a true gap nukes root',
      () {
        ...
        expect(
          host.contains('function frameIdAtPoint('),
          isTrue,
          reason: 'the host still needs a hit-test for the gap decision',
        );
        expect(
          host.contains('return true; // Card hit: popup.js owns'),
          isTrue,
          reason:
              'a card hit defers to popup.js (no host post -> no double-fire)',
        );
        expect(
          host.contains('function postFrameStampedTapOutside('),
          isFalse,
          reason:
        ...
        expect(
          ...
          isTrue,
          reason:
              'a frame-stamped tapOutside (from popup.js) closes only that '
              'layer children',
        );
      },
    );
    ...
    test('子3 — reveal gate driven by popup.js popupRendered, not body height', () {
      ...
      expect(
        host.contains("message.handler === 'popupRendered'"),
        isTrue,
        reason: 'the wrapped bridge marks content-ready on popupRendered',
      );
      expect(
        host.contains('markContentReady(record, messageRoute)'),
        isTrue,
      ...
      expect(
        host.contains("doc.querySelector('.no-results')"),
        isTrue,
  ...
  group('JS harness (node)', () {
    test(
      'popup.css scoped-style structure (executes parser via node)',
      () async {
        ...
        final File jsTest = File(
          'test/lookup/global_lookup_popup_style_test.mjs',
        );
        ...
        expect(
          ...
          0,
          reason:
              'global-lookup popup style JS test failed.\n'
              'stdout:\n${result.stdout}\nstderr:\n${result.stderr}',
        );
        expect(
