  ...
  group('BUG-1079 extension self-update stale detection', () {
    mirrors.forEach((String name, String root) {
      test(
        ...
        () {
          ...
          // 只认「self-update.js 在 importScripts 参数里」：该调用会随功能增参
          // （BUG-2480 加了 site-cookie-export.js），钉整串字面量只会让无关改动翻红。
          final RegExp importScripts = RegExp(r'importScripts\(([^)]*)\)');
          final Match? call = importScripts.firstMatch(src);
          expect(
            call,
            isNotNull,
            reason: '$root background.js must importScripts',
          );
          expect(
            call!.group(1),
            contains("'self-update.js'"),
            reason: '$root background.js must load self-update.js',
          );
          expect(
            src.contains('FUSHI_SELF_UPDATE.decide('),
            isTrue,
            reason:
          ...
          expect(
            src.contains('fushiUpdateStale: decision.stale'),
            isTrue,
            reason:
                '$root background.js must persist the stale marker when a '
          ...
          expect(
            ...
            isTrue,
            reason:
                '$root background.js must clear the stale marker once '
                'builds match again',
          );
        },
      );
      ...
      test(
        ...
        () {
          final String src = File('$root/background.js').readAsStringSync();
          expect(
            src.contains('statusRequestBody()'),
            isTrue,
            reason:
                '$root background.js must send the self-reported '
                'build/version body on /api/extension/status',
          );
          expect(
            src.contains('chrome.runtime.getManifest().version'),
            isTrue,
      ...
      test('[$name] update badge yields to the recording badge', () {
        ...
        expect(
          body.contains('if (await isOffscreenRecording()) return;'),
          isTrue,
          reason:
              '$root refreshUpdateBadge must skip while recording '
              '(recording badge has priority)',
        );
        expect(
          src.contains('if (!on) refreshUpdateBadge();'),
          isTrue,
          reason:
              '$root setRecordingBadge(false) must restore the update '
    ...
    test('self-update.js mirrors stay byte-identical', () {
      final List<int> tools = File(
        '../tools/browser-extension/self-update.js',
      ).readAsBytesSync();
      final List<int> assets = File(
        'assets/browser_extension/self-update.js',
      ).readAsBytesSync();
      expect(
        assets,
        tools,
    ...
    test(
      'server parses the reported build from the status body (tolerant)',
      () {
        final String src = File(
          'lib/src/sync/yomitan_api_server.dart',
        ).readAsStringSync();
        expect(
          src.contains('onExtensionReport'),
          isTrue,
          reason: 'yomitan_api_server.dart must expose the report callback',
        );
        expect(
          src.contains('_handleExtensionStatus'),
          isTrue,
        ...
        expect(
          ...
          isTrue,
          reason:
              'server must only report a non-empty string build '
    ...
    test('app model records the reported build for the extension page', () {
      ...
      expect(
        src.contains('browserExtensionReportedBuild'),
        isTrue,
      ...
      expect(
        ...
        isTrue,
        reason:
            'app_model.dart must wire onExtensionReport into the '
    ...
    test('extension page shows both builds and a mismatch warning', () {
      ...
      expect(
        src.contains('browser_extension_version_app'),
        isTrue,
        reason: 'version card must label the app bundled build',
      );
      expect(
        src.contains('browser_extension_version_browser'),
        isTrue,
        reason: 'version card must label the browser-loaded build',
      );
      expect(
        src.contains('browser_extension_version_mismatch'),
        isTrue,
        reason: 'version card must show the mismatch guidance',
      );
      expect(
        src.contains('build != null && reported != null && build != reported'),
        isTrue,
        reason:
            'mismatch warning must require both builds known and unequal '
