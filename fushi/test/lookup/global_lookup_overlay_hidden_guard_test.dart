...
void main() {
    ...
    test('native overlayHidden 调用派发到 onOverlayHidden 回调', () async {
      ...
      await messenger.handlePlatformMessage(
        ...
        codec.encodeMethodCall(
          const MethodCall('jsMessage', '{"handler":"x"}'),
        ),
        (_) {},
      );
  ...
  group('三端接线（源码扫描）', () {
    test('native Hide(bool) 只在真实关闭时 fire hidden_cb_', () {
      ...
      expect(
        cpp.contains('void GlobalLookupWindow::Hide(bool notify)'),
        isTrue,
      ...
      // 就永远停在"还开着"。
      expect(
        cpp.contains('const bool was_showing = visible_ || offscreen_active_;'),
        isTrue,
        reason: '必须在清 visible_ 前捕获（含离屏路由），避免双钩子双通知',
      );
      expect(
        cpp.contains('if (notify && was_showing && hidden_cb_)'),
        isTrue,
    ...
    test(
      ...
      () {
        ...
        expect(
          cb.contains('const GlobalLookupWindow::RouteContext& route'),
          isTrue,
          reason: 'hidden 回调必须接收 RouteContext',
        );
        expect(
          cb.contains('RoutedHiddenEnvelope(route)'),
          isTrue,
          reason: 'overlayHidden 必须带上路由信息回 Dart',
        );
        // "hide" 方法读 notify 参数转发给 Hide（通道按 target 路由后统一用 win）。
        expect(
          src.contains('win->Hide(BoolFromValue(args, "notify", true))'),
          isTrue,
    ...
    test(
      ...
      () {
        ...
        expect(
          c.contains('onOverlayHidden: _onOverlayHidden'),
          isTrue,
          reason: 'setHandlers 必须接上 _onOverlayHidden',
        );
        expect(
          c.contains('void Function()? onHidden;'),
          isTrue,
        ...
        expect(
          c.contains('void _onOverlayHidden([GlobalLookupRoute? routed])'),
          isTrue,
        ...
        //     暂停的视频当成「用户关了卡」恢复播放。
        expect(
          'GlobalLookupChannel.hide(notify: false)'.allMatches(c).length,
          5,
        ...
        expect(
          'GlobalLookupChannel.hide();'.allMatches(c).length,
          4,
