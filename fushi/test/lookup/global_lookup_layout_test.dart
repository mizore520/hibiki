...
void main() {
  group('horizontal isVertical false', () {
    ...
    test(
      'nested height ignores anchor-side space and only fits screen bounds',
      () {
    ...
    test('game-sized nested card stays wholly above or below its word', () {
      // SGRE's current SINGLE-CARD cap is 1592x1020 at dpr=2 (796x510 CSS),
      // but the cascade work area is the FULL 3840x2160 game viewport
      // (1920x1080 CSS). The word rect starts parent-local and is lifted by the
      // fixed root origin before layout.
      const Rect root = Rect.fromLTWH(562, 500, 796, 510);
      const Rect parentLocalWord = Rect.fromLTWH(136, 434, 20, 28);
      final Rect anchor = parentLocalWord.shift(root.topLeft);
      final GlobalLookupFrameRect child = computeFrameRect(
        selectionRect: anchor,
        screenW: 1920,
        screenH: 1080,
        maxWidth: 796,
      ...
      expect(child.top + child.height, lessThanOrEqualTo(anchor.top));
      expect(child.top, greaterThanOrEqualTo(6));
      expect(child.height, 510);
      expect(child.left, greaterThanOrEqualTo(6));
      expect(child.left + child.width, lessThanOrEqualTo(1914));
      expect(
        child.top,
        lessThan(root.top),
        reason: 'the child may use viewport space outside the root card',
      );
      expect(
        root.contains(Offset(child.left, child.top)) &&
            root.contains(
              Offset(child.left + child.width, child.top + child.height),
            ),
        isFalse,
        reason: 'the old 60% red rectangle must not constrain the whole stack',
      );
    });
  });
  ...
  group('vertical isVertical true', () {
    test('case4a ample right: popup to the right of selection', () {
      ...
      expect(
        r.top,
        closeTo(294, 1e-9),
      ); // centerY uses height/2: rawY=300+150=450 clamp[156,444]->444 top=294
    });
  ...
  group('TODO-893 screenH must be the real screen, not the measurement canvas', () {
    ...
    test('real screen-work-area screenH keeps the child BELOW the word', () {
      ...
      expect(
        r.top,
        greaterThanOrEqualTo(lowSel.bottom),
        reason: 'with the real screen the card stays below (no upward shove)',
      );
    });
    ...
    test('a word high on the real screen also drops below', () {
      ...
      expect(
        r.top,
        greaterThanOrEqualTo(sel.bottom),
  ...
  group('TODO-893 pickScreenDim wiring (work area beats measurement canvas)', () {
    ...
    test(
      'work area unreported (0, native query failed) -> measurement canvas',
      () {
  ...
  group('TODO-893 _renderStack source wiring guard', () {
    test(
      'screenW/screenH are sourced via pickScreenDim(_screenWork*, ...)',
      () {
        ...
        expect(
          ...
          isTrue,
          reason:
              'screenW must come from the work area first via pickScreenDim',
        );
        expect(
          body.contains('pickScreenDim(_screenWorkH, _layoutBoundsH, cardH)'),
          isTrue,
          reason:
              'screenH must come from the work area first via pickScreenDim',
        );
      },
    );
  });
  ...
  group('TODO-893 v2 coordinate-domain (near screen bottom edge)', () {
    ...
    test('WRONG domain (raw window-local selY) mis-decides showBelow=true', () {
      ...
      expect(
        wrong.top,
        greaterThan(windowLocalAnchor.bottom),
    ...
    test('FIXED domain (anchor lifted by cursorWorkY) flips to ABOVE', () {
      ...
      expect(
        windowLocalTop + r.height,
        lessThanOrEqualTo(windowLocalAnchor.top),
  ...
  group('GlobalLookupFrameRect data class', () {
    test('centerX/centerY derived plus value equality', () {
      ...
      expect(a.centerX, 60);
      expect(a.centerY, 45);
      const GlobalLookupFrameRect b = GlobalLookupFrameRect(
        left: 10,
        top: 20,
        width: 100,
        height: 50,
  ...
  group('TODO-1345/TODO-1231 (BUG-583/BUG-670) computeCascadeHeadroomSeed', () {
    ...
    test(
      'no work area reported (0) degenerates to no reservation (pre-fix)',
      () {
        final ({double left, double top}) s = computeCascadeHeadroomSeed(
          cursorWorkX: 0,
          cursorWorkY: 0,
        ...
        expect(
          s.left,
          0,
          reason: 'no work area -> no floor (pre-fix geometry)',
        );
        expect(s.top, 0);
      },
    );
  ...
  group('TODO-1231 (BUG-670) floor covers every cascade card (geometric guard)', () {
    ...
    void assertFloorCoversAllAnchors({
      ...
      for (double ax = 0; ax <= screenW; ax += 160) {
        for (double ay = 0; ay <= screenH; ay += 130) {
          ...
          expect(
            windowLocalLeft,
            greaterThanOrEqualTo(floor.left - 1e-6),
            reason:
                'anchor ($ax,$ay) card left must be >= reserved floor '
                '(vertical=$isVertical cursor=$cursorWorkX,$cursorWorkY)',
          );
          expect(
            windowLocalTop,
            greaterThanOrEqualTo(floor.top - 1e-6),
            reason:
                'anchor ($ax,$ay) card top must be >= reserved floor '
                '(vertical=$isVertical cursor=$cursorWorkX,$cursorWorkY)',
          );
        }
      }
    ...
    test(
      'vertical cascade: floor covers all cards from a bottom-right cursor',
      () {
        assertFloorCoversAllAnchors(
          cursorWorkX: 1700,
          cursorWorkY: 900,
  ...
  group(
    'TODO-1231 computeRootShellOffset (root card clamped into work area)',
    () {
      const double screenW = 1920;
      const double screenH = 1040;
      ...
      test('REGRESSION LOCK: cursor near the RIGHT edge -> pulled back left by '
      ...
      test(
        'REGRESSION LOCK: cursor near the BOTTOM edge -> pulled back up',
        () {
      ...
      test('card wider/taller than the work area -> pinned at the near edge '
      ...
      test(
        'no work area reported (screenWork <= 0) -> (0,0), pre-fix geometry',
        () {
          final ({double left, double top}) o = computeRootShellOffset(
            cursorWorkX: 0,
            cursorWorkY: 0,
      ...
      test('cursorWork beyond the work far edge (cursor over a bottom taskbar) '
          '-> clamped back inside', () {
        ...
        () {
          for (double cx = 0; cx <= screenW; cx += 128) {
            for (double cy = 0; cy <= screenH; cy += 104) {
              ...
              final ({double left, double top}) floor =
              ...
              expect(
                o.left,
                greaterThanOrEqualTo(floor.left - 1e-9),
                reason:
                    'cursor ($cx,$cy): root offset must stay inside the '
                    'reserved floor so the window origin never moves',
              );
              expect(
                o.top,
                greaterThanOrEqualTo(floor.top - 1e-9),
                reason:
                    'cursor ($cx,$cy): root offset must stay inside the '
                    'reserved floor so the window origin never moves',
              );
              ...
              expect(
                rootLeft + cardW,
                lessThanOrEqualTo(screenW + 1e-9),
                reason:
                    'cursor ($cx,$cy): clamped root card must fit the work '
                    'area horizontally',
              );
              expect(
                rootTop + cardH,
                lessThanOrEqualTo(screenH + 1e-9),
                reason:
                    'cursor ($cx,$cy): clamped root card must fit the work '
                    'area vertically',
              );
  ...
  group('TODO-1231 root clamp render wiring guard', () {
    test(
      ...
      () {
        ...
        expect(
          body.contains("'left': rootShellOffset.left,"),
          isTrue,
          reason: 'the anchorless branch must base its left on the root clamp',
        );
        expect(
          body.contains("'top': rootShellOffset.top + offset,"),
          isTrue,
    ...
    test('game route enables anchor-side nested height fitting only there', () {
      final File render = File('lib/src/lookup/global_lookup_render.dart');
      final String renderBody = render.readAsStringSync();
      expect(
        renderBody.contains('depth <= 0 || fitNestedHeightToAnchorSide'),
        isTrue,
        reason:
            'the render layer must allow the gal route to restore the '
            'in-app above/below height contract',
      );
      final File controller = File(
        'lib/src/lookup/global_lookup_controller.dart',
      );
      final String controllerBody = controller.readAsStringSync();
      expect(
        controllerBody.contains(
          "fitNestedHeightToAnchorSide: route.source == 'galCard'",
        ),
        isTrue,
        reason:
            'desktop global lookup must keep its existing cascade while '
            'the game-card route opts into anchor-side fitting',
      );
      expect(
        controllerBody.contains(
        ...
        isTrue,
        reason:
            'a non-zero game root must not reserve a transparent union back to viewport 0,0',
      );
    });
  });
}
