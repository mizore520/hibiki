...
void main() {
  ...
  test(
    'continuous pointer drag captures reader body text; paged protects it',
    () {
      ...
      expect(
        containsCodeLine(guard, '_fushiReaderPointerPrimaryButton(e)'),
        isTrue,
      );
      expect(
        containsCodeLine(guard, "e.pointerType !== 'mouse'"),
        isFalse,
        reason: 'touch and mouse must share the same drag state machine',
      );
      expect(
        containsCodeLine(guard, "closest('a[href], ruby, rt, rp')"),
        isTrue,
        reason: 'links and ruby text must keep native selection/click behavior',
      );
      expect(
        containsCodeLine(guard, 'input, textarea, select, button'),
        isTrue,
        reason: 'form controls must keep native editing and selection',
      );
      expect(
        containsCodeLine(guard, '[contenteditable="true"]'),
        isTrue,
        reason: 'editable islands must not be grabbed for reader scrolling',
      );
      expect(
        containsCodeLine(guard, 'window.getSelection'),
        isTrue,
      ...
      expect(
        containsCodeLine(guard, 'if (fushiContinuousMode) return false;'),
        isTrue,
        reason: '连续模式鼠标左键回归原生选字/划词，不再捕获正文拖动平移（已砍）',
      );
      expect(
        containsCodeLine(guard, 'if (fushiContinuousMode) return true;'),
        isFalse,
        reason: '旧的「连续模式捕获正文拖动平移」已砍，防回归',
      );
      final int continuousModeIndex = guard.indexOf(
        'if (fushiContinuousMode) return false;',
      );
      final int readerTextHitIndex = guard.indexOf(
        'window.fushiSelection.getCharacterAtPoint',
      );
      final int caretHitIndex = guard.indexOf(
        'return !_fushiReaderCaretRangeAtPoint',
      );
      expect(
        readerTextHitIndex,
        greaterThan(continuousModeIndex),
      ...
      expect(
        containsCodeLine(pointerDown, "e.pointerType === 'touch' ||"),
        isFalse,
        reason: 'touch must not be filtered out of pointer drag startup',
      );
      final int guardIndex = pointerDown.indexOf(
        '_fushiReaderMouseDragStartAllowed(e)',
      );
  ...
  test(
    ...
    () {
      ...
      expect(
        containsCodeLine(pointerMove, "e.pointerType === 'touch') return"),
        isFalse,
        reason: 'touch moves must be able to claim reader scrolling',
      );
      expect(
        containsCodeLine(pointerMove, '_fushiReaderPointerStillDown(e)'),
        isTrue,
      );
      expect(
        containsCodeLine(pointerMove, '_fushiReaderMouseDragClaimed = true'),
        isTrue,
      );
      expect(
        containsCodeLine(
          pointerMove,
          '_fushiReaderMouseDragIgnoreTouchEnd = true',
        ),
        isTrue,
        reason:
      ...
      expect(
        containsCodeLine(pointerMove, '_fushiReaderClearMouseSelection()'),
        isTrue,
        reason: 'claimed text drags must clear browser native selection',
      );
      expect(
        containsCodeLine(pointerMove, '_fushiReaderPointerNoSelect(true)'),
        isTrue,
        reason:
      ...
      expect(
        containsCodeLine(pointerUp, "e.pointerType === 'touch' ||"),
        isFalse,
        reason: 'touch pointerup must finish the same claimed-drag path',
      );
      final int finishIndex = pointerUp.indexOf(
      ...
      expect(
        finishIndex,
        lessThan(gestureEndIndex),
        reason: 'claimed drags must finish before the legacy _gestureEnd path',
      );
      expect(
        containsCodeLine(pointerUp, 'if (_fushiReaderMouseDragClaimed)'),
        isTrue,
      );
      expect(
        containsCodeLine(pointerUp, '_fushiReaderPointerNoSelect(false)'),
        isTrue,
      );

      final String touchEnd = _jsListener(setupScript, 'touchend');
      final int ignoreIndex = touchEnd.indexOf(
        '_fushiReaderMouseDragIgnoreTouchEnd',
      );
      ...
      expect(
        ignoreIndex,
        lessThan(legacyEndIndex),
        reason: 'claimed touch drags must not replay tap/selection on touchend',
      );
  ...
  test(
    'continuous pointer drag scrolls along horizontal and vertical axes',
    () {
      ...
      expect(
        containsCodeLine(scrollFn, 'window.scrollBy({left: 0, top:'),
        isTrue,
      );
      ...
      expect(
        containsCodeLine(pointerMove, "callHandler('onSwipe'"),
        isFalse,
  ...
  test('continuous vertical drag follows the pointer without a sign flip', () {
    ...
    expect(
      containsCodeLine(scrollFn, 'window.scrollBy({left: -dx, top: 0'),
      isTrue,
      reason:
          'vertical drag must pan with scrollBy({left: -dx}) so the '
          'content follows the pointer (mouse-right → content-right)',
    );
    // Horizontal axis stays finger-following on the vertical pointer axis.
    expect(
      containsCodeLine(scrollFn, 'window.scrollBy({left: 0, top: -dy'),
      isTrue,
    ...
    expect(
      containsCodeLine(scrollFn, '-dx * sign'),
      isFalse,
      reason: 'BUG-338: the writing-mode sign flip reversed vertical-rl drag',
    );
    expect(
      containsCodeLine(scrollFn, "=== 'vertical-rl') ? -1"),
      isFalse,
  ...
  test('paged desktop mouse drag emits at most one onSwipe on release', () {
    ...
    expect(
      containsCodeLine(finishFn, '_fushiReaderMouseDragSwipeSent'),
      isTrue,
    );
    expect(
      containsCodeLine(finishFn, '_fushiReaderMouseDragSwipeSent = true'),
      isTrue,
    );
    ...
    expect(
      ...
      isFalse,
      reason:
          'paged mouse drag decides direction during move but sends once '
          'from pointerup',
    );
  });
  ...
  test(
    ...
    () {
      ...
      // 左键现在也可以是阅读器页面动作的绑定；它只在命中注册表时由 Dart
      // 执行，不阻止正文自身的 click/选词链。查词弹窗另有自己的左键保留边界。
      expect(
        containsCodeLine(mouseDown, 'if (e.button === 0) return;'),
        isFalse,
      );
      expect(
        containsCodeLine(mouseDown, 'if (e.button !== 0) e.preventDefault();'),
        isTrue,
      );
      expect(
        containsCodeLine(mouseDown, 'e.button === 2 && _fushiBlockImageUrl'),
        isTrue,
      );
      expect(
        containsCodeLine(mouseDown, "callHandler('onPointerSeek'"),
        isTrue,
      );
    },
  );
  ...
  test('touch only engages the pointer drag machine in continuous mode', () {
    ...
    expect(
      containsCodeLine(engages, '_fushiReaderPointerPrimaryButton(e)'),
      isTrue,
    );
    expect(containsCodeLine(engages, "e.pointerType === 'touch'"), isTrue);
    expect(
      containsCodeLine(engages, 'return fushiContinuousMode'),
      isTrue,
    ...
    final String pointerDown = _jsListener(setupScript, 'pointerdown');
    expect(
      containsCodeLine(pointerDown, '_fushiReaderPointerEngages(e)'),
      isTrue,
    ...
    expect(
      containsCodeLine(
        pointerMove,
        "e.pointerType === 'touch' && !fushiContinuousMode",
      ),
      isTrue,
      reason:
          'paged-mode touch moves must return before claiming a drag, '
          'leaving touchend -> _gestureEnd -> onSwipe to turn the page',
    );

    final String pointerUp = _jsListener(setupScript, 'pointerup');
    expect(
      containsCodeLine(pointerUp, '_fushiReaderPointerEngages(e)'),
      isTrue,
    ...
    expect(
      ...
      isTrue,
      reason:
          'paged-mode touch pointercancel must bail before resetting the '
...
String _jsListener(String source, String eventName) =>
    methodBody(source, "'$eventName', function(e)", lexicon: SourceLexicon.js);
