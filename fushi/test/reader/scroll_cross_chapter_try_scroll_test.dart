...
void main() {
  ...
  group('JS 接线守卫：触摸/滚轮跨章不再用瞬时几何', () {
    ...
    test('_bStart 记手势起点滚动量 downSPos/downSMax', () {
      expect(
        containsCodeLine(paginationScripts, 'downSPos'),
        isTrue,
        reason: 'touchstart 必须记手势起点沿内容轴的滚动量',
      );
      expect(
        containsCodeLine(paginationScripts, 'downSMax'),
        isTrue,
    ...
    test('_bEnd 用手势起点在边界判据，不再用 touchend 瞬时 atTop', () {
      expect(
        containsCodeLine(paginationScripts, 'downAtStart'),
        isTrue,
        reason: '跨章必须看手势起点是否在章首（downSPos<=2）',
      );
      expect(
        containsCodeLine(paginationScripts, 'downAtEnd'),
        isTrue,
        reason: '跨章必须看手势起点是否在章末',
      );
      expect(
        containsCodeLine(paginationScripts, 'var atTop = root.scrollTop <= 2'),
        isFalse,
    ...
    test('滚轮跨章用真试滚（scrollBy + moved），不再用 stuck 推算/瞬时几何', () {
      ...
      // 说清是守卫自己锚歪了。
      expect(
        wheel.contains('fushiContinuousMode'),
        isTrue,
        reason:
            '窗口锚到了没有连续/分页门控的那份 wheel 监听（spread 独立文档），'
            '守卫在守错对象',
      );
      expect(
        wheel.contains('onSpreadTapEmpty'),
        isFalse,
