...
/// (Clipboard.setData) 与「查词」(_handleTextSelected) 两条出口；原地未拖动也保留单字
/// 选区并弹菜单；绝不复活 TODO-1279 掉的原生双选区。
///
...
void main() {
  group('拖选松手弹选区菜单（复制/查词共存，不再强制查词）', () {
    ...
    test(
      'endRangeSelection 真拖动路由到 fireSelectionMenu，不再直接 fireTextSelected',
      () {
        ...
        expect(
          body,
          isNot(contains('this.fireTextSelected(')),
    ...
    test(
      ...
      () {
        expect(
          js,
          contains('fireSelectionMenu: function'),
          reason: '缺拖选菜单派发 API',
        );
        ...
        expect(
          menuBody,
          contains("callHandler('onSelectionMenu'"),
          reason: 'fireSelectionMenu 必须派发 onSelectionMenu 给 Dart 弹菜单',
        );

        final String tapBody = _between(
          js,
          'fireTextSelected: function',
          'fireSelectionMenu: function',
        );
        expect(
          tapBody,
          contains("callHandler('onTextSelected'"),
    ...
    test('查词与选区菜单共用单一 buildSelectionPayload（同构 payload 契约）', () {
      expect(
        js,
        contains('buildSelectionPayload: function'),
  ...
  group('Dart 端：选区菜单同时给「复制」与「查词」两条出口（共存）', () {
    test('webview.part.dart 注册 onSelectionMenu -> _handleSelectionMenu', () {
      ...
      expect(
        src,
        contains("handlerName: 'onSelectionMenu'"),
        reason: '缺 onSelectionMenu handler 注册',
      );
      expect(
        src,
        contains('_handleSelectionMenu(ReaderSelectionData'),
    ...
    test('chrome.part.dart _handleSelectionMenu 同时提供复制 + 查词', () {
      ...
      expect(
        body,
        contains('_handleTextSelected(data)'),
        reason: '选区菜单「查词」必须复用 _handleTextSelected（含制卡）',
      );
      // 「复制」出口：整段选区文本进剪贴板。
      expect(
        body,
        contains('Clipboard.setData(ClipboardData(text: data.text))'),
