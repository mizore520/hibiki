...
void main() {
  test('BUG-255: InAppWebViewManager releases dcomp compositor on controlled '
    ...
    expect(
      header.contains('instance_count_'),
      isTrue,
      reason:
          '必须有 instance_count_ 统计存活的 InAppWebViewManager 实例数，'
          '只在最后一个实例析构时释放进程级共享单例',
    );
    expect(
      src.contains('++instance_count_'),
      isTrue,
    ...
    expect(
      header.contains('releaseSharedCompositionResources'),
      isTrue,
    ...
    final int dtorStart = src.indexOf(
    ...
    final int relStart = src.indexOf(
      'void InAppWebViewManager::releaseSharedCompositionResources',
    );
    ...
    final int dqcNull = relBody.indexOf(
    ...
    expect(
      relBody.contains('graphics_context_ = nullptr'),
      isTrue,
  ...
  test('BUG-289: shared dcomp compositor released on root WM_DESTROY (controlled '
    ...
    final File src = sourceCandidates
        .map(File.new)
        .firstWhere((File f) => f.existsSync());
    final File header = headerCandidates
        .map(File.new)
        .firstWhere((File f) => f.existsSync());
    ...
    expect(
      s.contains('RegisterTopLevelWindowProcDelegate'),
      isTrue,
      reason:
    ...
    final int releaseAfterWm = s.indexOf(
      'releaseSharedCompositionResources',
      wmDestroyIdx,
    );
    expect(
      releaseAfterWm,
      greaterThan(wmDestroyIdx),
    ...
    expect(
      relBody.contains('composition_released_'),
      isTrue,
      reason:
  ...
  test('TODO-489: exit(0) paths run an idempotent native pre-exit hook before '
    ...
    expect(
      RegExp(
        r"MethodChannel\(\s*'com\.pichillilorenzo/flutter_inappwebview_manager'\s*,?\s*\)",
      ).hasMatch(preExitSrc),
      isTrue,
      reason:
    ...
    expect(
      preExitSrc.contains("'prepareForProcessExit'"),
      isTrue,
    ...
    // 退出路径仍幂等，但两条路径互不短路。
    expect(
      preExitSrc.contains('static bool _prepared = false'),
      isFalse,
      reason:
          'TODO-618: close/update paths must not share a single one-shot guard.',
    );
    expect(
      preExitSrc.contains('enum WindowsExitReason'),
      isTrue,
      reason: 'Exit reason is the per-path guard key (update vs windowClose).',
    );
    expect(
      preExitSrc.contains('_preparedReasons.add(reason)'),
      isTrue,
      reason:
    ...
    final int lifecycleHook = lifecycleCode.indexOf(
      'WindowsNativePreExit.prepareForExit(',
    );
    ...
    final int updaterHook = updaterCode.indexOf(
      'WindowsNativePreExit.prepareForExit(',
    );
    ...
    final int nativeMethodEnd = managerSrc.indexOf(
      'void InAppWebViewManager::',
      nativeMethod + 1,
    );
    ...
    expect(
      nativeBody.contains('webViews.clear()'),
      isTrue,
      reason:
          'Pre-exit must release active WebView platform views before dcomp.',
    );
    expect(
      nativeBody.contains('keepAliveWebViews.clear()'),
      isTrue,
      reason: 'Pre-exit must release keep-alive WebViews before dcomp.',
    );
    expect(
      nativeBody.contains('windowWebViews.clear()'),
      isTrue,
      reason: 'Pre-exit must release pending popup WebViews before dcomp.',
    );
    expect(
      nativeBody.contains('releaseSharedCompositionResources()'),
      isTrue,
      reason: 'Pre-exit must release shared DirectComposition/WinRT resources.',
    );
  });
}
