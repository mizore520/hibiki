...
void main() {
  ...
  group('source guards: controller wires library service (Task-6)', () {
    ...
    test('_propagateDictionaryDeleteToRemote routes live backend via '
      ...
      expect(
        src.contains('backend.deleteRemoteDictionary'),
        isTrue,
        reason: '删除传播必须对 InterconnectSyncBackend 调 live DELETE 端点',
      );
      expect(
        src.contains('backend is InterconnectSyncBackend'),
        isTrue,
