...
class _StringsEn implements BaseTranslations<AppLocale, _StringsEn> {
	...
	String get onboarding_pack_action_download_desc => 'Downloads the whole pack in the background, then imports it. Cancel anytime; it resumes from where it stopped.';
	String get onboarding_pack_action_import_existing_desc => 'The pack is already downloaded; this imports it. Choose Merge in the confirmation dialog and your existing data is left alone.';
	String get onboarding_pack_action_pick_desc => 'Already got the pack zip from somewhere else? Import it from disk and skip the download entirely.';
	String get onboarding_pack_action_website => 'Open the download page on the website';
	String get onboarding_pack_action_website_desc => 'Opens the official site in your browser. Its pack section lists chunk links you can feed to a download manager; come back afterwards and use Choose a local pack file to import what you got.';
	String get onboarding_pack_action_dictionary_desc => 'Learning a language other than Japanese? Skip the pack and import dictionaries for your own language here instead.';
	String get onboarding_pack_action_audio_desc => 'Where pronunciation audio comes from. The pack already covers Japanese and English; add online sources here for other languages.';
	String get onboarding_anki_action_test_desc => 'Checks that Fushi can reach Anki and loads your decks and note types. Nothing is created yet.';
	String get onboarding_anki_action_refresh_desc => 'Reloads decks and note types from Anki. Use it after creating a new deck inside Anki.';
	String get onboarding_anki_action_get_ankidroid_desc => 'Opens the store page for AnkiDroid. Fushi writes its cards into it, so it has to be installed first.';
	String get onboarding_anki_action_get_anki_desc => 'Opens the Anki download page. Install Anki and keep it running while you make cards.';
	String get onboarding_anki_action_install_addon_desc => 'Unpacks the bundled AnkiConnect add-on into Anki for you, which is what lets Fushi talk to it. Restart Anki afterwards.';
	String get onboarding_step_anki_action_desc => 'Card template, field mapping, screenshots and audio: the details of what a created card looks like. The deck and note type above are enough to start, so open this only if you want to change how cards are built.';
	String get onboarding_step_backup_action_desc => 'Choose a backup backend and sign in, so your library survives a lost or replaced device.';
	String get onboarding_step_interconnect_action_desc => 'Pair this device with your other devices to share one library and keep progress in sync.';
	String get onboarding_step_extension_action_desc => 'Shows how to install the browser extension and connect it to Fushi, so you can look words up on web pages too.';
	String get onboarding_step_fonts_action_desc => 'Add your own font files and pick which one each language uses.';
	String get onboarding_pack_sources_hint => 'Downloaded in parallel chunks from GitHub, the official site and a backup mirror at the same time, each chunk checksummed. Fushi measures the sources as it goes and gives more chunks to whichever is fastest right now, so there is nothing to choose here.';
	...
	String get network_proxy_p2p_label => 'Route P2P (torrent) traffic through the proxy';
	...
	String get onboarding_click_lookup_nested_title => 'Keep exploring in the popup';
	String get onboarding_click_lookup_nested_body => 'Tap another word inside a definition to open a deeper lookup. Go back or tap outside to close one level.';
	String get onboarding_click_lookup_mine_title => 'Turn the result into a card';
	String get onboarding_click_lookup_mine_body => 'When the definition looks right, tap + to send the word, sentence, audio and image to the card creator.';
	String get onboarding_step_global_lookup_title => 'Look up text outside Fushi';
	String get onboarding_global_lookup_windows_body => 'On Windows, select text in another app and summon the dictionary without switching back to Fushi.';
	String get onboarding_global_lookup_windows_select_title => 'Select text in any app';
	String get onboarding_global_lookup_windows_shortcut_title => 'Press Ctrl+Alt+D';
	String get onboarding_global_lookup_windows_shortcut_body => 'This is the default global shortcut. Fushi captures the current selection and opens a lookup card near the pointer.';
	String get onboarding_global_lookup_windows_customize_title => 'Change the shortcut if needed';
	String get onboarding_global_lookup_windows_customize_body => 'Open Settings > Shortcuts > Global (app-external) to assign a different key combination.';
	String get onboarding_global_lookup_windows_action => 'Open shortcut settings';
	String get onboarding_global_lookup_windows_action_desc => 'Lets you change the app-external lookup shortcut. The default Ctrl+Alt+D already works, so this is optional.';
	String get onboarding_global_lookup_android_body => 'On Android, the system passes selected text to Fushi through the text menu or Share sheet. There is no remappable global hotkey.';
	String get onboarding_global_lookup_android_select_title => 'Select text in another app';
	String get onboarding_global_lookup_android_open_title => 'Choose Fushi';
	String get onboarding_global_lookup_android_open_body => 'Tap Fushi in the text-selection menu. If it is hidden, tap Share and choose Fushi from the share sheet.';
	String get onboarding_global_lookup_android_continue_title => 'Use the standalone popup';
	String get onboarding_global_lookup_android_continue_body => 'The lookup opens separately from the source app. You can tap more words inside it, then close it to return where you were.';
	String get onboarding_feature_manual_resources => 'Import dictionaries and audio manually';
	String get onboarding_step_manual_resources_title => 'Prepare dictionaries and audio manually';
	String get onboarding_manual_dictionary_action => 'Import a dictionary';
	String get onboarding_manual_dictionary_action_desc => 'Open dictionary management and import at least one supported dictionary file or archive. Lookup tutorials are useful only after a query returns a definition.';
	String get onboarding_manual_audiobook_action => 'Import a book with audiobook audio';
	String get onboarding_manual_audiobook_action_desc => 'Open book import and choose the book or text, aligned subtitles, and one or more audio files. Audio needs subtitles so Fushi can align it to sentences.';
	String get onboarding_manual_pronunciation_action => 'Configure word pronunciation audio';
	String get onboarding_manual_pronunciation_action_desc => 'Add local or online pronunciation sources used by dictionary entries. This is separate from audiobook audio attached to a book.';
	String get onboarding_lookup_verify_action => 'Verify a word in your dictionary';
	String get onboarding_lookup_verify_action_desc => 'Open lookup, enter any word you are learning, and continue only after the installed dictionary returns a definition. The tutorial does not hard-code a sample word.';
	String get onboarding_step_first_anki_card_title => 'Create your first Anki card';
	String get onboarding_step_first_anki_card_body => 'This step appears only after this onboarding session has connected to Anki and selected a usable deck and note type.';
	String get onboarding_first_anki_lookup_title => 'Start from a real dictionary result';
	String get onboarding_first_anki_lookup_body => 'Look up a word that your installed dictionary actually defines. There is no fixed tutorial word that might be absent from your dictionary.';
	String get onboarding_first_anki_plus_title => 'Tap the plus button on the entry';
	String get onboarding_first_anki_plus_body => 'The plus button opens the card creator with the current word, reading, meaning, sentence, audio, and available image context.';
	String get onboarding_first_anki_save_title => 'Review and save';
	String get onboarding_first_anki_save_body => 'Confirm the target deck, note type, and field preview, then save. Open Anki to verify that the first card arrived.';
	String get onboarding_first_anki_action => 'Open lookup and create a card';
	String get onboarding_first_anki_action_desc => 'Use a word with a visible definition, tap its plus button, review the fields, and save it to the connected Anki deck.';
	String get onboarding_step_click_lookup_body => 'First verify a word that your installed dictionary actually defines. Then use that same word to practise direct lookup in books, manga OCR text, and video subtitles.';
	String get onboarding_click_lookup_tap_body => 'On a phone, tap a character in the verified word. On a computer, left-click it. Fushi starts there and matches the longest word.';
	String get onboarding_global_lookup_windows_select_body => 'Highlight the same word you already verified has a dictionary definition, and leave the selection active.';
	String get onboarding_global_lookup_android_select_body => 'Long-press the same word you already verified has a dictionary definition, then adjust the selection handles to cover it.';
	String get onboarding_feature_manual_resources_hint => 'Supplement the recommended pack, or import your own dictionaries, audiobooks, and pronunciation sources';
	String get onboarding_step_manual_resources_body => 'Use this alongside the recommended pack or on its own. Import at least one dictionary before the lookup tutorial; audiobook and pronunciation audio are optional supplements.';
