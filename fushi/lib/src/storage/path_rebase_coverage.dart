...
const List<PathRebaseColumn> kPathRebaseColumns = <PathRebaseColumn>[
  ...
  PathRebaseColumn(
    ...
    'externalUrl',
    PathRebaseKind.notAPath,
    'provider 条目详情页的远端 URL，不是本机路径。',
  ),
  PathRebaseColumn(
    'VideoMetadataRawSnapshots',
    'rawJson',
    PathRebaseKind.notAPath,
    'provider 原始响应的结构化 JSON，不承载本机数据根路径。',
  ),
  PathRebaseColumn(
    'VideoMetadataImages',
    'remoteUrl',
  ...
  PathRebaseColumn(
    ...
    'resourceProvider',
    PathRebaseKind.notAPath,
    '资源 provider/实例身份，不是路径。',
  ),
  PathRebaseColumn(
    'VideoDownloadJobs',
    'selectedResourceId',
    PathRebaseKind.notAPath,
    'provider 侧资源 ID，不是路径。',
  ),
  PathRebaseColumn(
    'VideoDownloadJobs',
    'resourceTitle',
  ...
  PathRebaseColumn(
    ...
    'filterJson',
    PathRebaseKind.notAPath,
    '严格版本规则 JSON（组、分辨率、编码、语言），不承载路径或凭据。',
  ),
  PathRebaseColumn(
    'VideoDownloadSubscriptions',
    'identityJson',
  ...
  PathRebaseColumn(
    ...
    'extensionListUrl',
    PathRebaseKind.notAPath,
    '仓库可选的远端扩展清单地址，不是本机路径。',
  ),
  PathRebaseColumn(
    'MangaExtensionStores',
    'contactJson',
    PathRebaseKind.notAPath,
    '仓库维护者联系方式 JSON（远端字段），无本机路径。',
  ),
  PathRebaseColumn(
    'MangaExtensions',
    'storeUrl',
  ...
  PathRebaseColumn(
    ...
    'valueJson',
    PathRebaseKind.notAPath,
    '扩展自定义偏好值的 JSON 编码（扩展私有语义），无本机路径。',
  ),
];
...
final List<PathRebasePref> kPathRebasePrefs = <PathRebasePref>[
  ...
  PathRebasePref(
    ...
    PathRebaseKind.externalUserPath,
    PathValueShape.none,
    '用户本机 mpv 着色器目录，外部路径，不随数据根走。',
  ),
  PathRebasePref(
    'manga_external_mokuro_path',
    PathRebaseKind.externalUserPath,
    PathValueShape.none,
    '系统安装的 mokuro 可执行文件路径，外部路径。',
  ),
  PathRebasePref(
  ...
  PathRebasePref(
    ...
    PathRebaseKind.notAPath,
    PathValueShape.none,
    '当前 Profile 的自增 id（撞正则是因为 profile 里含 file）。不是路径。',
  ),
  PathRebasePref(
    'local_audio_db_display_name',
    PathRebaseKind.notAPath,
    PathValueShape.none,
    '旧单库显示名（与 local_audio_db_path 同族），不是路径。',
  ),
];
