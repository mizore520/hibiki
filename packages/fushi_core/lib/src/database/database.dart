...
Future<QueryExecutor> _openWithRecovery(
  ...
  try {
    ...
    debugPrint(
      '[fushi-db] sidecar open error on "$path" '
  ...
  if (!allowSidecarDelete) {
    throw FushiDatabaseUnrecoverableException(
      dbPath: path,
      cause:
          'sidecar open error in a non-main (:popup) process; backing off '
...
Future<void> _rebuildSidecar(File dbFile) async {
  ...
  debugPrint(
    '[fushi-db] Layer 2: deleted stale -wal/-shm for "$path" '
...
/// `.db` / `.db-wal` / `.db-shm`）。
final RegExp _kCorruptBakSuffix = RegExp(
  r'^\.corrupt-bak-\d+\.db(-wal|-shm)?$',
);
...
List<File> listDatabaseSnapshotFiles(final Directory supportRoot) {
  ...
  final List<FileSystemEntity> children = supportRoot.listSync(
    followLinks: false,
  );
...
Future<void> _migrateLegacyDatabaseFileName(String dbDirectory) async {
  ...
  debugPrint(
    '[fushi-db] renamed legacy hibiki.db(+sidecars) -> fushi.db '
    'in "$dbDirectory"',
  );
}
...
_MergedTagState _mergeTagClocks(
  ...
  for (final MapEntry<String, int> e in remoteAdded.entries) {
    add[e.key] = add.containsKey(e.key)
        ? _maxInt(add[e.key]!, e.value)
        : e.value;
  }
  final Map<String, int> tomb = <String, int>{...localTomb};
  for (final MapEntry<String, int> e in remoteTomb.entries) {
    tomb[e.key] = tomb.containsKey(e.key)
        ? _maxInt(tomb[e.key]!, e.value)
        : e.value;
  }
...
class FushiDatabase extends _$FushiDatabase
  ...
  MigrationStrategy get migration => MigrationStrategy(
    onUpgrade: (m, from, to) async {
      ...
      if (from < 28) {
        ...
        if (await _tableExists('video_books') &&
            !await _columnExists('video_books', 'secondary_subtitle_source')) {
      ...
      if (from < 40) {
        ...
        if (await _tableExists('media_collections') &&
          ...
          await m.addColumn(mediaCollections, mediaCollections.orderUpdatedAt);
        }
      ...
      if (from < 41) {
        ...
        if (await _tableExists('video_book_tag_mappings') &&
          ...
          await customStatement(
            'ALTER TABLE video_book_tag_mappings '
            'ADD COLUMN added_at INTEGER NOT NULL DEFAULT 0',
          );
        }
      ...
      if (from < 52) {
        ...
        if (await _tableExists('media_collections')) {
          ...
          if (!await _columnExists('media_collections', 'subtitle_delay_ms')) {
            await m.addColumn(
              mediaCollections,
              mediaCollections.subtitleDelayMs,
            );
          }
        }
      }
      ...
      if (from < 68) {
        ...
        if (!await _tableExists('media_images')) {
          ...
          if (await _tableExists('collection_scrape_meta') &&
              await _columnExists('collection_scrape_meta', 'backdrop_path')) {
      ...
      if (from < 72) {
        ...
        if (await _tableExists('epub_books')) {
          await customStatement(
            'UPDATE epub_books SET extract_dir = '
        ...
        if (await _tableExists('media_items')) {
          await customStatement(
            'UPDATE media_items SET image_url = '
      ...
      if (from < 73) {
        ...
        for (final String table in <String>[
          ...
          if (!await _tableExists(table)) continue;
          await customStatement(
            'UPDATE OR REPLACE $table SET key = '
          ...
          for (final QueryRow r in legacyRows) {
            final Map<String, Object?> snapshot = <String, Object?>{
              ...
              if (r.read<String?>('audio_url') case final String v)
                'audioUrl': v,
              if (r.read<String?>('author') case final String v) 'author': v,
      ...
      if (from < 83 && await _tableExists('epub_books')) {
        ...
        if (await _tableExists('media_collection_items')) {
          ...
          await customStatement(
            'ALTER TABLE media_collection_items_v83 '
      ...
      if (from < 86) {
        ...
        if (await _tableExists('media_collections') &&
            !await _columnExists(
              'media_collections',
              'secondary_subtitle_delay_ms',
            )) {
          await m.addColumn(
            mediaCollections,
            mediaCollections.secondarySubtitleDelayMs,
          );
        }
      }
      if (from < 87) {
        ...
        if (await _tableExists('dictionary_metadata') &&
          ...
          await m.addColumn(
            dictionaryMetadata,
            dictionaryMetadata.languageOverride,
          );
        }
        if (await _tableExists('epub_books') &&
      ...
      if (from < 91) {
        ...
        if (await _tableExists('media_collections') &&
          ...
          await m.addColumn(
            mediaCollections,
            mediaCollections.subtitleLanguage,
          );
        }
        if (await _tableExists('media_collections') &&
            !await _columnExists(
          ...
          await m.addColumn(
            mediaCollections,
            mediaCollections.subtitleReleaseGroup,
          );
        }
      }
      if (from < 92) {
    ...
    beforeOpen: (details) async {
      ...
      if (_isMainProcess && await _tableExists('video_source_scrape_runs')) {
        final int interruptedAt = DateTime.now().millisecondsSinceEpoch;
        await (update(videoSourceScrapeRuns)..where(
  ...
  Future<void> _rewriteTextPrefix({
    ...
    final String likePrefix = sqlQuote(
      from,
  ...
  Future<bool> _videoBooksKeyedByBookUid() async {
    ...
    final Iterable<QueryRow> pkCols = rows.where(
      (QueryRow r) => r.read<int>('pk') > 0,
    );
  ...
  Future<void> backfillMismatchedAudiobookKeysV26() async {
    ...
    final bool hasAudioCues =
        await _tableExists('audio_cues') &&
  ...
  Future<void> splitPlaylistVideoBooksV38() async {
    ...
    await transaction(() async {
      ...
      final Set<String> taken = <String>{
        for (final QueryRow r in await customSelect(
      ...
      for (final QueryRow row in playlistRows) {
        ...
        final String? secondarySub = row.read<String?>(
...
/// bookKey）；跨设备无需一致。
String generateEpubBookUid() =>
    'book_${DateTime.now().microsecondsSinceEpoch}_'
    '${_epubBookUidCounter++}';
