...
class LocalLibraryHostService extends _LocalLibraryHostBase
  ...
  LocalLibraryHostService({
    ...
    Future<VideoSourceScrapeTaskController?> Function()? scrapeController,
        ...
        _adoptOverrideTitle = adoptOverrideTitle,
  ...
  /// 推送方随书带来的显示名 → host 本机 override（LWW）。app 注入
  /// `ReaderFushiSource.adoptOverrideTitleIfNewer`（要写穿 MediaSource 内存缓存）；
  /// 无头服务端注入只写 DB 的实现；null = 不采纳（书仍入库）。
  @override
