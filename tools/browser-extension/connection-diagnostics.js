(function (root, factory) {
  ...
  function copy(state, port) {
    ...
    if (state === states.connected) {
      return { title: '已连接 Hibiki', detail: '查词、字幕解析和制卡服务均可用。', tone: 'good' };
    }
    if (state === states.legacy) {
      return { title: '已连接旧版 Hibiki', detail: '基础功能可用；更新 Hibiki 可获得完整连接诊断。', tone: 'good' };
    }
    if (state === states.unauthorized) {
      return { title: 'API 密钥不匹配', detail: '连接到了 Hibiki，但扩展中的 Token 已过期。请点“恢复自动配置”或重新运行安装助手。', tone: 'warn' };
    }
    if (state === states.yomitanConflict) {
      return {
        title: '端口被 Yomitan API 占用',
        detail: '端口 ' + p + ' 正由 Yomitan 使用。请在 Yomitan 高级设置中关闭“Enable Yomitan API”，再回 Hibiki 开启“Yomitan API 服务器”。',
        tone: 'danger',
      };
    }
    if (state === states.wrongService) {
      return { title: '端口连接到了其他服务', detail: '端口 ' + p + ' 有响应，但不是 Hibiki。请检查端口设置或关闭占用该端口的程序。', tone: 'danger' };
    }
    return {
      title: 'Hibiki API 未开启',
      detail: '请打开 Hibiki → 设置 → 查词 → 开启“Yomitan API 服务器”，然后重新检测。',
      tone: 'warn',
    };
  }
